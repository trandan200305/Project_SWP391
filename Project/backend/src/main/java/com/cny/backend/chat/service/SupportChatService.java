...
public class SupportChatService {
    ...
    public List<ChatMessageDto> getChatHistory(Integer ticketId) {
        try {
            jdbcTemplate.update("UPDATE ticket_messages SET is_read = 1 WHERE ticket_id = ?", ticketId);
        } catch (Exception e) {
            System.err.println("Lỗi khi cập nhật is_read: " + e.getMessage());
        }
    ...
    public List<Map<String, Object>> getTicketsByFreelancerId(Integer freelancerId) {
        if (freelancerId == null || freelancerId <= 0) {
            throw new IllegalArgumentException("Mã freelancer không hợp lệ.");
        }

        String sql = "SELECT t.ticket_id, t.freelancer_id, t.subject, t.description, t.status, t.priority, t.created_at, t.updated_at, t.assigned_staff_id, " +
                     "s.display_name as staff_name, s.avatar_url as staff_avatar, " +
                     "(SELECT TOP 1 message_text FROM ticket_messages WHERE ticket_id = t.ticket_id ORDER BY sent_at DESC) as last_message, " +
                     "(SELECT TOP 1 sent_at FROM ticket_messages WHERE ticket_id = t.ticket_id ORDER BY sent_at DESC) as last_message_at, " +
                     "(SELECT COUNT(*) FROM ticket_messages WHERE ticket_id = t.ticket_id AND (sender_admin_id IS NOT NULL OR sender_staff_id IS NOT NULL) AND is_read = 0) as unread_count " +
        ...
        List<Map<String, Object>> rows = jdbcTemplate.queryForList(sql, freelancerId);
        List<Map<String, Object>> tickets = new ArrayList<>();
    ...
    @Transactional
    public Integer createFreelancerTicket(Integer freelancerId, String subject, String description, String priority, List<Map<String, Object>> attachments) {
        if (freelancerId == null || freelancerId <= 0) {
            throw new IllegalArgumentException("Mã freelancer không hợp lệ.");
        }

        // Validate Freelancer Existence
        Integer countFreelancer = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM freelancers WHERE freelancer_id = ?", Integer.class, freelancerId);
        if (countFreelancer == null || countFreelancer == 0) {
            throw new IllegalArgumentException("Tài khoản freelancer không tồn tại trong hệ thống.");
        }
        ...
        String insertTicketSql = "INSERT INTO support_tickets (freelancer_id, subject, description, status, priority, created_at, updated_at) " +
                                 "VALUES (?, ?, ?, 'OPEN', ?, GETDATE(), GETDATE())";
        jdbcTemplate.update(insertTicketSql, freelancerId, subj, desc, p);
        Integer ticketId = jdbcTemplate.queryForObject("SELECT IDENT_CURRENT('support_tickets')", Integer.class);

        String insertMsgSql = "INSERT INTO ticket_messages (ticket_id, sender_freelancer_id, message_text, is_read, sent_at) " +
                              "VALUES (?, ?, ?, 0, GETDATE())";
        jdbcTemplate.update(insertMsgSql, ticketId, freelancerId, desc);
        Integer messageId = jdbcTemplate.queryForObject("SELECT IDENT_CURRENT('ticket_messages')", Integer.class);
