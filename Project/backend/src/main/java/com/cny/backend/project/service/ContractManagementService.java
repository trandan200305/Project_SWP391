...
public class ContractManagementService {
    ...
    @Transactional
    public void fileDispute(Integer contractId, Integer freelancerId, CreateDisputeDto dto) {
        Contract contract = contractRepository.findById(contractId)
                .orElseThrow(() -> new IllegalArgumentException("Không tìm thấy hợp đồng ID: " + contractId));
        ...
        java.math.BigDecimal amount = dto.getAmount() != null ? dto.getAmount() : contract.getAgreedAmount();
        if (amount.compareTo(contract.getAgreedAmount()) > 0) {
            throw new IllegalArgumentException("Số tiền khiếu nại không được lớn hơn tổng giá trị hợp đồng.");
        }
        String priority = dto.getPriority() != null ? dto.getPriority() : "HIGH";
