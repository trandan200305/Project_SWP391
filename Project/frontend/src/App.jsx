...
export default function App() {
    ...
    const handleNavigate = (page, params = null) => {
        ...
        // 2. Kiểm tra quyền Admin (ở đây chắc chắn user khác null nên dùng trực tiếp user.role)
        if (page === 'admin' && !['ADMIN', 'STAFF', 'MANAGER'].includes(user.role)) {
            setCurrentPage('coming_soon');
            return;
        }

        // 3. Gộp các trang yêu cầu quyền EMPLOYER
        if (['post_job', 'employer_profile'].includes(page) && user.role !== 'EMPLOYER') {
            if (page === 'post_job') {
