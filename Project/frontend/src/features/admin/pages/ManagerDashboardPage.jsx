...
export default function ManagerDashboardPage({
  ...
  const activeDeptId = (() => {
    ...
    let code = "ALL";
    if (email.startsWith("manager.mod") || email.startsWith("staff.moderation"))
      code = "MOD";
  ...
  const handleTransferRequestSubmit = (e) => {
    ...
    adminApi
      .transferDepartmentMember(payload)
      .then((res) => {
      ...
      .catch((err) => {
        ...
        showToast(
          ...
          "Có lỗi xảy ra khi thực hiện điều chuyển.",
          "error",
        );
      });
  };
  ...
  const currentDeptStaffList = staffList.filter((s) => {
    if (activeDeptCode === "ALL") return true;
    if (activeDeptId && s.departmentId === activeDeptId) return true;
    const code = getStaffDeptCode(s);
    if (code && code === activeDeptCode) return true;
    const email = String(s.email || "").toLowerCase();
    if (
      activeDeptCode === "MOD" &&
      (email.includes("mod") || email.includes("moderation"))
    )
      return true;
    if (
      activeDeptCode === "DIS" &&
      (email.includes("dis") || email.includes("dispute"))
    )
      return true;
    if (
      activeDeptCode === "FIN" &&
      (email.includes("fin") || email.includes("finance"))
    )
      return true;
    if (
      activeDeptCode === "CS" &&
      (email.includes("cs") || email.includes("support"))
    )
      return true;
    if (
      activeDeptCode === "IT" &&
      (email.includes("it") || email.includes("tech"))
    )
      return true;
    return false;
  });
  ...
  const getSidebarItems = () => {
    ...
    if (activeDeptCode === "MOD") {
      ...
      common.push(
        {
          ...
          badge: withdrawals.filter((w) => w.statusRaw === "PENDING").length,
        },
        { id: "FailedTransactions", label: "Đối soát PayOS", icon: Activity },
      );
      ...
      common.push({
        ...
        icon: MessageSquare,
        badge: supportChats.reduce((sum, c) => sum + (c.unread || 0), 0),
      });
    } else if (activeDeptCode === "IT") {
      common.push({
        id: "FailedTransactions",
        label: "Đối soát & Sự cố kỹ thuật",
        icon: Activity,
        badge: vnpayTxns?.filter(
          (tx) =>
    ...
    common.push({
      ...
      icon: Users,
      badge: currentDeptStaffList.length,
    });
  ...
  // trang thai khoi luong cong viec
  const fetchTasks = () => {
    adminApi
        ...
        if (Array.isArray(data)) {
          const mapped = data.map((t) => {
            ...
            return {
              ...
              user:
                t.assignedToEmail || t.verifierEmail || `Dept: ${firstDept}`,
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          const mapped = data.map((r) => ({
            ...
            docUrl:
              r.idCard ||
              "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=400&fit=crop",
            status:
              r.status === "APPROVED"
                ? "Approved"
                : r.status === "REJECTED"
        ...
        if (Array.isArray(gigsData)) {
          mapped = [
            ...
            ...gigsData.map((g) => ({
              ...
              reason: "Dịch vụ mới",
              subDate: g.createdAt
                ? String(g.createdAt).substring(0, 10)
  ...
  const parseReason = (reasonText) => {
    ...
    lines.forEach((line) => {
      if (line.startsWith("Lý do điều chuyển: ")) {
        ...
        line.startsWith("Kỹ năng liên quan & kinh nghiệm trước đây: ")
      ) {
        data.skills = line.substring(
          "Kỹ năng liên quan & kinh nghiệm trước đây: ".length,
        );
      } else if (line.startsWith("Thành tích nổi bật & lý do bạn phù hợp: ")) {
        data.achievements = line.substring(
          "Thành tích nổi bật & lý do bạn phù hợp: ".length,
        );
  ...
  // tu choi don duyet don
  const executeTransferRequestAction = (requestId, status, reason) => {
    const adminId = user?.id || 1;
    adminApi
      .approveTransferRequest(requestId, status, reason, adminId)
      .then((res) => {
        if (res.success || res.success !== false) {
          showToast(
            res.message || "Xử lý đơn điều chuyển thành công!",
            "success",
          );
          fetchTransferRequests();
      ...
      .catch((err) => {
        console.error(err);
        showToast(
          err.response?.data?.message || "Không thể kết nối máy chủ.",
          "error",
        );
      });
  };
  ...
  const handleApproveTransferRequest = (requestId, approve) => {
    if (!approve) {
      ...
      const req =
        transferRequests.find((r) => r.requestId === requestId) ||
      ...
      const unfinishedCount = tasks.filter((t) => {
        const isAssigned =
          (t.assignedToEmail &&
            t.assignedToEmail.toLowerCase() === staffEmail.toLowerCase()) ||
          (t.assignedTo &&
            t.assignedTo.toLowerCase() === staffEmail.toLowerCase());
        const isFinished =
          t.status === "Completed" ||
      ...
      setConfirmConfig({
        title:
          unfinishedCount > 0
            ? "⚠️ Cảnh báo bàn giao công việc"
            : "Xác nhận duyệt điều chuyển",
        message:
          unfinishedCount > 0
        ...
        confirmText:
          unfinishedCount > 0 ? "Gửi yêu cầu bàn giao" : "Xác nhận Duyệt",
        cancelText: "Hủy",
  ...
  const handleManagerResolveReport = (report, status) => {
    const rawId =
      report.idRaw ||
      (report.id ? String(report.id).replace("RPT-", "") : report.id);
    const adminId = user?.id || 1;
    setReportActionLoading(true);
    adminApi
      .resolveReport(rawId, status, adminId)
      .then((res) => {
        setReportActionLoading(false);
        if (res.success !== false) {
          showToast(
            status === "RESOLVED"
              ? "Đã phê duyệt và ký duyệt báo cáo thành công!"
  ...
  const handleResolveDispute = (status) => {
    ...
    adminApi
      ...
      .then((res) => {
        if (res.success) {
          showToast(
            res.message || "Đã xử lý tranh chấp thành công.",
            "success",
          );
          setShowDisputeModal(false);
          setSelectedDispute(null);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("openTransferRequestDetail", handleOpenDetail);
  }, [transferRequests]);
  ...
  useEffect(() => {
    const handleOpenProfileDetail = (e) => {
      ...
      if (Array.isArray(moderationItems)) {
        const found = moderationItems.find(
          (item) => item.type === "PROFILE" && item.idRaw === requestId,
        );
        if (found) {
  ...
  const handleCreateTaskSubmit = (e) => {
    ...
    if (!createForm.assignedToEmail || !createForm.assignedToEmail.trim()) {
      showToast("Vui lòng chọn nhân viên để bàn giao công việc!", "error");
      return;
    }

    const payload = {
      taskType: createForm.taskType || "PROFILE_MODERATION",
      referenceId: parseInt(createForm.referenceId || "1", 10),
      title: createForm.title.trim(),
      description: createForm.description ? createForm.description.trim() : "",
      requiredDepartments:
        createForm.requiredDepartments || activeDeptCode || "MOD",
      assignedToEmail: createForm.assignedToEmail || null,
    };
    // gui thong bao
    adminApi
      .createVerificationTask(payload)
      .then((res) => {
        if (
          res &&
          (res.success || res.taskId || res.id || res.success !== false)
        ) {
          showToast("Tác vụ xác thực đã được phân công thành công!", "success");
          fetchTasks();
          setShowCreateModal(false);
          setCreateForm({
            taskType: "PROFILE_MODERATION",
            title: "",
            requiredDepartments: activeDeptCode || "MOD",
            description: "",
            referenceId: "",
            assignedToEmail: "",
          });
  ...
  const handleInviteStaff = (e) => {
    ...
    adminApi
      .inviteStaffOrManager(
        inviteForm.email.trim(),
        inviteForm.role,
        inviteForm.departmentId,
        user?.id || 1,
      )
      .then((res) => {
        if (res.success !== false) {
  ...
  const handleUpdateTaskStatus = (id, newStatus) => {
    ...
    adminApi
      .submitTaskSignoff(
        ...
        {
          status:
            newStatus === "Completed" || newStatus === "Manager đã ký duyệt"
              ? "APPROVED"
              : "PENDING",
  ...
  const handleAssignTask = (taskId, staffEmail) => {
    setIsLoading(true);
    adminApi
      .claimVerificationTask(taskId, staffEmail)
      .then((res) => {
        setIsLoading(false);
        if (res.success) {
  ...
  const handleKycAction = (idRaw, approve, role) => {
    adminApi
      .moderateKycRequest(idRaw, approve, role)
      .then((res) => {
        if (res.success) {
          showToast(
  ...
  const handleModAction = (item, approve) => {
    ...
    if (item.type === "PROJECT") {
      ...
      apiCall = Promise.resolve({
        success: true,
        message: approve ? "Đã phê duyệt mục (Demo)" : "Đã từ chối mục (Demo)",
      });
    }
    ...
    apiCall
      .then((res) => {
        if (res.success) {
          showToast(
            res.message ||
            (approve ? "Đã phê duyệt thành công!" : "Đã từ chối thành công!"),
            approve ? "success" : "error",
          );
          fetchModerationItems();
        } else {
  ...
  const countCompleted = tasks.filter(
    (t) => t.status === "Manager đã ký duyệt",
  ).length;
  ...
  const selectedTaskMatchingItem = selectedTask
    ? moderationItems.find(
      (item) =>
        ...
        (selectedTask.type === "PROJECT_MODERATION"
          ? item.type === "PROJECT"
          : item.type === "PROFILE"),
    )
    : null;
              ...
              {getSidebarItems().map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-body-sm font-semibold transition-all duration-200 group relative ${isActive
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <IconComp
                        className={`w-[18px] h-[18px] stroke-[2.2] transition-colors ${isActive
                          ? "text-[#006b2c]"
                          : "text-[#6e7b6c] group-hover:text-[#141b2b]"
                          }`}
                      />
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
                ...
                <div className="profile-menu-dropdown">
                  ...
                  <div className="profile-menu-item">
                    <button
                      ...
                      className={`profile-menu-btn w-full flex items-center gap-3 px-3 py-2.5 text-sm font-semibold rounded-xl transition-all mt-1 ${activeTab === "preferences"
                  ...
                  {user?.role !== "STAFF" && user?.role !== "MANAGER" && (
                    <div className="profile-menu-item">
                      <button
                        ...
                        className={`profile-menu-btn w-full flex items-center gap-3 px-3 py-2.5 text-sm font-semibold rounded-xl transition-all mt-1 ${activeTab === "messenger"
                          ? "profile-menu-active text-emerald-600 bg-emerald-50"
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-[#f9f9ff]">
            ...
            <div className="space-y-6 max-w-7xl mx-auto pb-10">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-body-sm text-[#6e7b6c] mt-1">
                    Ghi nhận hiệu suất vận hành thời gian thực của phòng ban.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {/* Stat 1: Total Staff */}
                <div
                  className="bg-white border border-[#e1e8fd] p-5 rounded-xl flex flex-col justify-between min-h-[120px] card-level-1 cursor-pointer hover:border-[#006b2c] transition-all"
                  onClick={() => setActiveTab("Staff Management")}
                >
                  <div className="w-10 h-10 rounded-lg bg-[#f7fff2] text-[#006b2c] flex items-center justify-center mb-2">
                    <Users className="w-5 h-5" />
                  </div>
                  <div>
                    <p className="text-xs font-bold text-[#6e7b6c] uppercase tracking-wider">
                      TỔNG NHÂN SỰ
                    </p>
                    <h2 className="text-3xl font-extrabold text-[#141b2b] mt-1">
                      {currentDeptStaffList.length}
                    </h2>
                  </div>
                </div>

                {/* Stat 2: Pending Cases */}
                <div
                  className="bg-white border border-[#e1e8fd] p-5 rounded-xl flex flex-col justify-between min-h-[120px] card-level-1 cursor-pointer hover:border-[#0058be] transition-all"
                  onClick={() => setActiveTab("Dashboard")}
                >
                  <div className="w-10 h-10 rounded-lg bg-[#f1f3ff] text-[#0058be] flex items-center justify-center mb-2">
                ...
                {/* Stat 3: Transfer Requests */}
                <div
                  className="bg-white border border-[#e1e8fd] p-5 rounded-xl flex flex-col justify-between min-h-[120px] card-level-1 cursor-pointer hover:border-rose-300 transition-all"
                  onClick={() => setActiveTab("Staff Management")}
                >
                  <div className="w-10 h-10 rounded-lg bg-[#ffdad6] text-[#ba1a1a] flex items-center justify-center mb-2">
                    <ShieldAlert className="w-5 h-5" />
                  </div>
                  <div>
                    <p className="text-xs font-bold text-[#6e7b6c] uppercase tracking-wider">
                      ĐƠN ĐIỀU CHUYỂN CHỜ DUYỆT
                    </p>
                    <h2 className="text-3xl font-extrabold text-[#141b2b] mt-1">
                      {
              ...
              <div className="w-full">
                <div className="bg-white border border-[#e1e8fd] p-6 rounded-xl w-full card-level-1 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="overflow-x-auto mt-4">
                      <table className="min-w-full divide-y divide-[#e9edff] text-left">
                        <thead>
                          <tr>
                            <th className="pb-3 text-[11px] font-bold text-[#6e7b6c] uppercase tracking-wider">
                              Nhân viên
                            </th>
                            <th className="pb-3 text-[11px] font-bold text-[#6e7b6c] uppercase tracking-wider text-center">
                              Việc đang làm
                            </th>
                            <th className="pb-3 text-[11px] font-bold text-[#6e7b6c] uppercase tracking-wider">
                              Tiến độ tải
                            </th>
                            <th className="pb-3 text-[11px] font-bold text-[#6e7b6c] uppercase tracking-wider text-right">
                              Hiệu suất
                            </th>
                          </tr>
                        </thead>
                              ...
                              <td className="py-3.5 whitespace-nowrap">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-[10px] font-semibold text-[#6e7b6c]">
                                      {item.role}
                                    </p>
                                  </div>
                                </div>
          ...
          {activeTab === "Staff Management" &&
            (() => {
              const deptStaffList = currentDeptStaffList;
              ...
              const deptTransferRequests = transferRequests.filter((r) => {
                ...
                const isStaffInMyDept = deptStaffList.some(
                  (s) =>
                    (s.email &&
                      ...
                      String(r.userEmail).toLowerCase()) ||
                    (s.id && r.userId && String(s.id) === String(r.userId)) ||
                    (s.staffId &&
                      r.userId &&
                      String(s.staffId) === String(r.userId)),
                );
                ...
                if (
                  ...
                  (email.includes("moderation") ||
                    email.includes("staff.mod") ||
                    email.includes("staff.cs") ||
                    email.includes("staff.it"))
                ) {
                  return false;
                }
                ...
                const fromCode = String(
                  r.fromDepartmentCode || r.fromDeptCode || r.fromCode || "",
                ).toUpperCase();
                ...
                const fromName = String(
                  r.fromDepartmentName || r.fromDepartment || "",
                ).toLowerCase();
                let targetKeywords = [];
                if (currentCode === "DIS")
                  targetKeywords = ["tranh chấp", "dispute", "khiếu nại"];
                else if (currentCode === "MOD")
                  targetKeywords = ["kiểm duyệt", "moderation"];
                else if (currentCode === "FIN")
                  targetKeywords = ["tài chính", "finance"];
                else if (currentCode === "CS")
                  targetKeywords = ["hỗ trợ", "customer support", "cs"];
                ...
                <div className="space-y-6 max-w-7xl mx-auto">
                  <div className="flex items-center justify-between">
                    <div>
                      <h1 className="text-headline-lg font-extrabold text-[#141b2b]">
                        Quản lý Nhân sự & Điều chuyển
                      </h1>
                      <p className="text-body-sm text-[#3e4a3d] mt-1">
                        Danh sách nhân viên phòng ban và xử lý các đơn xin điều
                        chuyển công việc.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex border-b border-[#e1e8fd] gap-6 text-sm font-semibold">
                    <button
                      ...
                      className={`pb-3 transition-colors relative ${staffSubTab === "list"
                        ? "text-[#006b2c] border-b-2 border-[#006b2c] font-bold"
                    ...
                    <button
                      ...
                      className={`pb-3 transition-colors relative flex items-center gap-1.5 ${staffSubTab === "requests"
                        ? "text-[#006b2c] border-b-2 border-[#006b2c] font-bold"
                        : "text-slate-400 hover:text-slate-600"
                        }`}
                    >
                      Đơn xin điều chuyển ({deptTransferRequests.length})
                      {deptTransferRequests.filter(
                        (r) => r.status === "PENDING",
                      ).length > 0 && (
                      ...
                      <table className="min-w-full divide-y divide-[#e9edff] text-left">
                        <thead>
                          <tr className="bg-[#f9f9ff]">
                            <th className="px-6 py-3.5 text-label-md text-[#6e7b6c] uppercase tracking-wider">
                              Mã nhân sự
                            </th>
                            <th className="px-6 py-3.5 text-label-md text-[#6e7b6c] uppercase tracking-wider">
                              Họ và tên
                        ...
                        <tbody className="divide-y divide-[#e9edff] bg-white">
                          {deptStaffList.map((staff) => (
                            <tr
                              ...
                              className="hover:bg-[#f7fff2]/30 transition-colors"
                            >
                              <td className="px-6 py-4 text-body-sm font-bold text-[#006b2c]">
                                #STF-{staff.id || staff.staffId}
                              </td>
                              <td className="px-6 py-4 text-body-sm font-bold text-[#141b2b]">
                                {staff.email
                                  ? staff.email.split("@")[0]
                                  : staff.name || staff.fullName || "Nhân viên"}
                              </td>
                              <td className="px-6 py-4 text-body-sm text-[#3e4a3d]">
                                {staff.email}
                              </td>
                              <td className="px-6 py-4 text-body-sm text-[#3e4a3d] font-semibold">
                                {staff.departmentName || "Tổng bộ"}
                              </td>
                            </tr>
                          ))}
                        </tbody>
                    ...
                    (() => {
                      const getTransferStatusInfo = (req) => {
                        ...
                        if (
                          ...
                          (status === "APPROVED" && !isHandoverDone)
                        ) {
                          return {
                      ...
                      {
                        /* loc don xin dieu chuyen */
                      }
                      const filteredRequests = deptTransferRequests.filter(
                        (r) => matchesTransferTab(r, transferFilter),
                      );
                      return (
                        <div className="card-level-1 p-6 bg-white shadow-sm rounded-2xl border border-[#e1e8fd]">
                          {/* Filter buttons */}
                          <div className="mb-6 flex flex-wrap gap-2 items-center justify-between border-b border-slate-100 pb-5">
                            {/* nut bam don xin dieu chuyen */}
                            <div className="flex flex-wrap gap-2">
                              {[
                                ...
                                const count = deptTransferRequests.filter((r) =>
                                  matchesTransferTab(r, tab.value),
                                ).length;
                                return (
                                  <button
                                    key={tab.value}
                                    onClick={() => setTransferFilter(tab.value)}
                                    className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${transferFilter === tab.value
                                  ...
                                  >
                                    {tab.label}
                                    <span
                                      className={`px-1.5 py-0.5 rounded text-[10px] font-extrabold ${transferFilter === tab.value
                            ...
                            <tbody className="divide-y divide-[#e9edff] bg-white">
                              {filteredRequests.length === 0 ? (
                                <tr>
                                  ...
                                  >
                                    Không tìm thấy yêu cầu điều chuyển nào phù
                                    hợp.
                                  </td>
                                </tr>
                              ) : (
                                  ...
                                  >
                                    ...
                                    <td className="px-6 py-4">
                                      {(() => {
                                        ...
                                        return (
                                          <span
                                            className={`inline-flex px-2.5 py-1 rounded-full text-[10px] font-extrabold border ${info.colorClass}`}
                                          >
                                            {info.label}
                                          </span>
                                        );
                      ...
                      <div className="p-4 border-b border-[#e1e8fd] space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-1.5">
                          <button
                            ...
                            className={`flex-1 py-1 rounded-lg text-[10px] font-bold transition-all border text-center ${supportSubTab === "unclaimed"
                          ...
                          <button
                            ...
                            className={`flex-1 py-1 rounded-lg text-[10px] font-bold transition-all border text-center ${supportSubTab === "claimed"
                              ? "bg-[#006b2c] text-white border-[#006b2c] shadow-sm shadow-[#006b2c]/10"
                              : "bg-[#f1f3ff] text-[#3e4a3d] border-transparent hover:bg-[#e1e8fd]"
                              }`}
                          >
                          ...
                          <button
                            ...
                            className={`flex-1 py-1 rounded-lg text-[10px] font-bold transition-all border text-center ${supportSubTab === "blocked"
                          ...
                          <button
                            ...
                            className={`flex-1 py-1 rounded-lg text-[10px] font-bold transition-all border text-center ${supportSubTab === "deleted"
                      ...
                      <div className="flex-1 overflow-y-auto divide-y divide-[#e9edff] scrollbar-hidden">
                        {displayedChats.map((chat) => (
                          <button
                            ...
                            className={`w-full text-left p-4 flex gap-3 transition-colors ${selectedChatId === chat.id
                    ...
                    {!activeChat ? (
                      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-[#f9f9ff]">
                        ...
                        <p className="text-body-sm text-[#6e7b6c] max-w-xs leading-relaxed">
                          Choose a chat from the contact list on the left to
                          start live support messaging and user moderation.
                        </p>
                      </div>
                    ) : (
                      <div className="flex-1 flex flex-col bg-[#f9f9ff] min-w-0">
                        <div className="px-6 py-4 bg-white border-b border-[#e1e8fd] flex items-center justify-between shrink-0">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => setShowUserInfo(!showUserInfo)}
                              className={`p-2 rounded-lg transition-colors border border-[#e1e8fd] hover:bg-[#f1f3ff] ${showUserInfo
                                    ...
                                    {m.messageText &&
                                      ...
                                      !(
                                        ...
                                        (m.messageText === "[Hình ảnh]" ||
                                          m.messageText === "[Tệp đính kèm]")
                                      ) && (
                                        <div
                                          className={`px-4 py-2.5 rounded-2xl text-body-sm leading-relaxed ${isMe
                        ...
                        {activeChat.blocked_until &&
                          ...
                          activeChat.assigned_staff_id ||
                          activeChat.assignedStaffId
                        ) ? (
                          ...
                          <div className="flex items-center justify-center p-4 bg-slate-100 border-t border-[#e1e8fd] h-[76px] shrink-0">
                            ...
                            <span className="text-xs font-bold text-slate-600">
                              Hội thoại này đang được xử lý bởi quản lý/nhân
                              viên khác.
                            </span>
                          </div>
                        ) : (
                    ...
                    {showUserInfo && activeChat && (
                      <div className="w-80 border-l border-[#e1e8fd] bg-white flex flex-col h-full shrink-0 overflow-y-auto animate-in slide-in-from-right duration-200">
                        <div className="p-6 border-b border-[#e9edff] flex flex-col items-center">
                          ...
                          <span
                            className={`text-[10px] font-extrabold px-2 py-0.5 rounded border uppercase tracking-wider ${activeChat.sender_role === "EMPLOYER"
          ...
          {activeTab === "Moderation" &&
            (() => {
              ...
              const processedItems = moderationItems.filter(
                (item) => item.status !== "Pending",
              );
              const escalatedModerationTasks = tasks.filter(
                (t) =>
                  t.status === "Escalated" &&
                  (t.type === "PROJECT_MODERATION" ||
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                      {moderationTabs.map((tab) => (
                        <button
                          ...
                          className={`px-3 py-2 rounded-lg text-xs font-extrabold transition-all border ${moderationView === tab.id
                        ...
                        <div className="flex gap-2 overflow-x-auto">
                          {[
                            ...
                            <button
                              ...
                              className={`px-3 py-1.5 rounded-full text-[11px] font-bold transition-all whitespace-nowrap border ${queueTab === qTab.id
                            ...
                            >
                              <td className="px-4 py-4">
                                <div className="min-w-[220px]">
                                  ...
                                  <h4
                                    className="text-body-sm font-bold text-[#141b2b] mt-1.5 group cursor-pointer hover:text-[#006b2c] transition-colors"
                                    onClick={() => {
                                  ...
                                  <p className="text-xs text-[#6e7b6c] mt-0.5 line-clamp-1">
                                    {item.detail}
                                  </p>
                                </div>
                              </td>
                        ...
                        {escalatedModerationTasks.map((task) => (
                          ...
                          >
                            <div className="flex justify-between items-start mb-2">
                              <span
                                className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${task.priority === "High"
                        ...
                        {escalatedModerationTasks.length === 0 && (
                          <div className="col-span-2 text-center py-10 bg-slate-50 rounded-xl border border-dashed border-[#bdcaba]">
                            <p className="text-sm text-[#6e7b6c]">
                              Không có trường hợp kiểm duyệt nào đang chuyển
                              cấp.
                            </p>
                          </div>
                        )}
              ...
              return (
                <div className="space-y-6 max-w-7xl mx-auto">
                  <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
                    <div>
                      <h1 className="text-headline-lg font-extrabold text-[#141b2b]">
                        Phê duyệt Xác thực KYC
                      </h1>
                      <p className="text-body-sm text-[#3e4a3d] mt-1">
                        Xem xét và phê duyệt hồ sơ xác minh danh tính cho
                        Freelancer và Nhà tuyển dụng.
                      </p>
                    </div>
                  ...
                  {filteredKyc.length === 0 ? (
                    <div className="bg-white border border-[#e1e8fd] rounded-xl p-10 text-center text-[#6e7b6c] text-sm">
                        ...
                        >
                          <div>
                            <div className="flex items-center justify-between pb-4 border-b border-[#e9edff]">
                              ...
                              <span
                                className={`px-2 py-0.5 rounded text-[10px] font-bold ${req.role === "FREELANCER"
                                  ? "bg-[#f7fff2] text-[#006b2c]"
                          ...
                          <div className="border-t border-[#e9edff] pt-4 flex items-center justify-between">
                            <span
                              className={`px-2.5 py-1 rounded-full text-xs font-bold ${req.status === "Approved"
                                ? "bg-[#f7fff2] text-[#006b2c]"
                                : "bg-amber-100 text-amber-800"
                                }`}
                            >
                            ...
                            {req.status === "Pending" ? (
                              <div className="flex items-center gap-3">
                                ...
                                <button
                                  onClick={() =>
                                    handleKycAction(req.idRaw, false, req.role)
                                  }
                                ...
                                <button
                                  onClick={() =>
                                    handleKycAction(req.idRaw, true, req.role)
                                  }
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {escalationCases.map((esc) => {
                        ...
                        return (
                          <div
                            key={esc.id}
                            className={`border rounded-xl p-4 transition-all hover:shadow-md ${isPending
                          ...
                          >
                            <div className="flex justify-between items-start mb-2">
                              <span
                                className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${esc.priority === "Khẩn cấp" ||
                                  esc.priority === "HIGH"
                                  ? "bg-rose-200 text-rose-800"
                                  : "bg-amber-100 text-amber-800"
                                  }`}
              ...
              const filteredReports = violationReports.filter((r) => {
                ...
                if (reportTypeFilter !== "ALL") {
                  if (reportTypeFilter === "PROJECT" && r.target !== "PROJECT")
                    return false;
                  if (reportTypeFilter === "USER" && r.target !== "USER")
                    return false;
                }
                // Search filter
                if (reportSearch) {
                        ...
                        <div className="flex bg-[#f1f3ff] p-1 rounded-lg">
                          {[
                            ...
                            <button
                              ...
                              className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${reportFilter === btn.key
                        ...
                        <div className="flex bg-[#f1f3ff] p-1 rounded-lg">
                          {[
                            ...
                            <button
                              ...
                              className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${reportTypeFilter === btn.key
                                ? "bg-white text-[#006b2c] shadow-sm"
                                : "text-[#6e7b6c] hover:text-[#141b2b]"
                                }`}
                            >
                      ...
                      {filteredReports.map((report) => (
                        ...
                        >
                          <div className="flex justify-between items-start mb-3">
                            ...
                            <span
                              className={`text-xs font-bold px-2 py-1 rounded ${report.status === "Chờ xử lý" ||
                                ...
                                <div>
                                  {(() => {
                                    ...
                                    return isAssigned ? (
                                      ...
                                      >
                                        <UserPlus className="w-3.5 h-3.5" /> Phân
                                        công
                                      </button>
                                    );
                                  })()}
                      ...
                      <div className="flex bg-[#f1f3ff] p-1 rounded-lg">
                        {[
                          ...
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${withdrawalFilter === tab.key
                              ? "bg-white text-[#006b2c] shadow-sm"
                              : "text-[#6e7b6c] hover:text-[#141b2b]"
                              }`}
                          >
                          ...
                          {filteredWds.length > 0 ? (
                            filteredWds.map((w) => (
                              ...
                              >
                                ...
                                <td className="px-5 py-4 whitespace-nowrap">
                                  <span
                                    className={`px-2 py-0.5 rounded text-[10px] font-bold ${w.statusRaw === "PENDING"
                                      ? "bg-amber-100 text-amber-800"
                                ...
                                {t.status === "FAILED" ? (
                                  <div className="flex items-center justify-end gap-2">
                                    <button
                                      ...
                                      className="px-3 py-1.5 bg-white hover:bg-[#006b2c] hover:text-white text-[#006b2c] border border-[#bdcaba] rounded-lg transition-colors cursor-pointer"
                                    >
                                      Đối soát lại
                                    </button>
                                    {(() => {
                                      const isAssigned = tasks.some(
                                        (task) =>
                                          (task.type === "FAILED_TRANSACTION" || task.taskType === "FAILED_TRANSACTION") &&
                                          Number(task.referenceId) === Number(t.id)
                                      );
                                      return isAssigned ? (
                                        <span className="px-2.5 py-1.5 bg-slate-100 text-slate-500 text-xs font-bold rounded-lg border border-slate-200">
                                          Đã phân công
                                        </span>
                                      ) : (
                                        <button
                                          onClick={() => {
                                            setCreateForm({
                                              taskType: "FAILED_TRANSACTION",
                                              title: `Đối soát sự cố giao dịch PayOS #${t.id} - ${t.txnRef}`,
                                              referenceId: t.id,
                                              description: `Yêu cầu kiểm tra đối soát và xử lý lại giao dịch lỗi có mã đối soát: ${t.txnRef}, số tiền: ${t.amount.toLocaleString("vi-VN")} VND.`,
                                              requiredDepartments: "IT",
                                              assignedToEmail: "",
                                            });
                                            setShowCreateModal(true);
                                          }}
                                          className="px-2.5 py-1.5 bg-[#f1f3ff] text-[#0058be] hover:bg-[#e1e8fd] border border-blue-200 font-bold text-xs rounded-lg transition-colors flex items-center gap-1 cursor-pointer"
                                        >
          ...
          <div className="bg-white rounded-xl w-full max-w-md p-6 shadow-xl border border-[#e1e8fd] animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleCreateTaskSubmit} className="mt-4 space-y-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-bold text-[#6e7b6c] uppercase">
                  Loại công việc
                </label>
                <select
                  disabled
                  ...
                  className="w-full bg-[#e9ecef] border border-transparent px-3 py-2 rounded-lg text-body-sm opacity-80 cursor-not-allowed border-[#e1e8fd] font-semibold text-[#141b2b]"
                >
                  <option value="KYC_VERIFICATION">Xác thực KYC</option>
                  <option value="WITHDRAWAL">Duyệt rút tiền</option>
                  <option value="DISPUTE_REFUND">
                    Giải quyết tranh chấp / Hoàn tiền
                  </option>
                  <option value="PROJECT_MODERATION">Kiểm duyệt dự án</option>
                  <option value="GIG_MODERATION">Kiểm duyệt dịch vụ</option>
                  <option value="PROFILE_MODERATION">Kiểm duyệt hồ sơ</option>
                  <option value="REPORT_RESOLUTION">
                    Xử lý báo cáo vi phạm
                  </option>
                  <option value="FAILED_TRANSACTION">
                    Đối soát & Sự cố giao dịch (IT)
                  </option>
                </select>
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  type="text"
                  placeholder="Ví dụ: Duyệt hồ sơ KYC cho Minh Anh"
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setCreateForm({
                        ...createForm,
                        requiredDepartments: e.target.value,
                      })
                    }
                    className="w-full bg-[#e9ecef] border border-transparent px-3 py-2 rounded-lg text-body-sm opacity-80 cursor-not-allowed border-[#e1e8fd] font-semibold text-[#141b2b]"
                  >
                    <option value="CS">Phòng CS (Hỗ trợ khách hàng)</option>
                    <option value="FIN">Phòng FIN (Tài chính)</option>
                    <option value="MOD">Phòng MOD (Kiểm duyệt)</option>
                    <option value="DIS">
                      Phòng DIS (Giải quyết tranh chấp)
                    </option>
                  </select>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-bold text-[#6e7b6c] uppercase flex items-center justify-between">
                  <span>Giao việc cho nhân viên</span>
                  <span className="text-[#ba1a1a] text-[10px] font-normal lowercase">* Bắt buộc chọn</span>
                </label>
                <select
                  required
                ...
                >
                  ...
                  {staffList
                    .filter((s) => {
                      if (
                        activeDeptCode !== "ALL" &&
                        String(s.departmentId) !== String(activeDeptId)
                      )
                        return false;
                      if (createForm.requiredDepartments) {
                        if (activeDeptCode !== "ALL") return true; // Manager assigning to own staff

                        const targetDeptMatch = departments.find(
                          (d) =>
                            String(d.code).toUpperCase() ===
                            createForm.requiredDepartments,
                        );
                        if (targetDeptMatch) {
                          return (
                            String(s.departmentId) ===
                            String(targetDeptMatch.id)
                          );
                        }
                        const staffCode = getStaffDeptCode(s);
                        if (staffCode)
                          return staffCode === createForm.requiredDepartments;
                      }
                      return true;
                    })
                    .map((s) => (
              ...
              <div className="flex gap-3 pt-3 border-t border-[#e9edff]">
                <button
                  type="button"
                  onClick={() => setShowCreateModal(false)}
                  className="flex-1 py-2 rounded-lg bg-slate-100 text-slate-700 font-bold text-body-sm"
                >
                  Đóng
                </button>
                {/* nut bam phan cong */}
                <button
                  type="submit"
                  className="flex-1 py-2 bg-[#006b2c] hover:bg-[#00873a] text-white rounded-lg font-bold text-body-sm shadow"
                >
                  Phân công
                </button>
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setTransferForm({
                      ...transferForm,
                      targetDepartmentCode: e.target.value,
                    })
                  }
                  className="w-full bg-[#f1f3ff] border border-transparent px-3 py-2 rounded-lg text-body-sm focus:outline-none focus:ring-2 focus:ring-[#006b2c]/30 focus:bg-white border-[#e1e8fd]"
                >
              ...
              <div className="py-6 space-y-4">
                      ...
                      {staffList
                        .filter((s) => {
                          if (
                            ...
                            return false;
                          const reqDepts =
                            selectedTask.requiredDepartments?.split(",") || [];
                          return (
                            reqDepts.length === 0 ||
                            reqDepts.some(
                              (d) => getStaffDeptCode(s) === d.trim(),
                            )
                          );
                        })
                        .map((s) => (
                ...
                <div className="grid grid-cols-2 gap-4 text-body-sm border-b border-[#e9edff] pb-4">
                  <div>
                    ...
                    <span className="block mt-1 font-bold text-sm text-[#ba1a1a]">
                  ...
                  <div>
                    ...
                    <span className="block mt-1 font-bold text-[#141b2b]">
                      {selectedTask.requiredDepartments}
                    </span>
                  </div>
                </div>
                ...
                {selectedTaskMatchingItem && (
                  <div className="pt-4 border-t border-[#e9edff] space-y-3">
                    <span className="text-xs font-bold text-[#6e7b6c] uppercase block text-left">
                      Nội dung kiểm duyệt (Content Details)
                    </span>

                    {selectedTaskMatchingItem.type === "PROFILE" &&
                      selectedTaskMatchingItem.rawRequest ? (
                      <div className="space-y-3 text-xs text-left">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-60 overflow-y-auto pr-1">
                          <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-1">
                            <p className="font-bold text-slate-500 uppercase pb-1.5 border-b border-slate-200 mb-1.5">
                              Thông tin hiện tại
                            </p>
                            <p>
                              <strong>Tên hiển thị:</strong>{" "}
                              {selectedTaskMatchingItem.rawRequest.employer
                            ...
                            <p>
                              ...
                              {selectedTaskMatchingItem.rawRequest.employer
                                ?.address
                                ? `${selectedTaskMatchingItem.rawRequest.employer.address}, ${selectedTaskMatchingItem.rawRequest.employer.city || ""}, ${selectedTaskMatchingItem.rawRequest.employer.country || ""}`
                                : "Chưa cập nhật"}
                            </p>
                            <p>
                              <strong>Mô tả:</strong>{" "}
                          ...
                          <div className="bg-indigo-50/30 p-3 rounded-xl border border-indigo-100 space-y-1">
                            <p className="font-bold text-indigo-600 uppercase pb-1.5 border-b border-indigo-100 mb-1.5">
                            ...
                            <p>
                              ...
                              >
                                {selectedTaskMatchingItem.rawRequest.fullName ||
                                  "Chưa cập nhật"}
                              </span>
                            </p>
                            <p>
                              ...
                              <span
                                className={
                                  selectedTaskMatchingItem.rawRequest.phone !==
                                    selectedTaskMatchingItem.rawRequest.employer
                                      ?.phone
                                    ? "text-indigo-655 font-bold"
                                    : ""
                            ...
                            <p>
                              ...
                              >
                                {selectedTaskMatchingItem.rawRequest.website ||
                                  "Chưa cập nhật"}
                              </span>
                            </p>
                            <p>
                            ...
                            <p>
                              ...
                              >
                                {selectedTaskMatchingItem.rawRequest.industry ||
                                  "Chưa cập nhật"}
                              </span>
                            </p>
                            <p>
                              ...
                              >
                                {selectedTaskMatchingItem.rawRequest.taxCode ||
                                  "Chưa cập nhật"}
                              </span>
                            </p>
                            <p>
                              ...
                              <span
                                className={
                                  selectedTaskMatchingItem.rawRequest
                                    ...
                                    selectedTaskMatchingItem.rawRequest.employer
                                      ?.address ||
                                    selectedTaskMatchingItem.rawRequest.city !==
                                    selectedTaskMatchingItem.rawRequest.employer
                                      ?.city
                                    ? "text-indigo-655 font-bold"
                                    : ""
                              ...
                              >
                                {selectedTaskMatchingItem.rawRequest.address
                                  ? `${selectedTaskMatchingItem.rawRequest.address}, ${selectedTaskMatchingItem.rawRequest.city || ""}, ${selectedTaskMatchingItem.rawRequest.country || ""}`
                                  : "Chưa cập nhật"}
                              </span>
                            </p>
                            <p>
                              ...
                              <span
                                className={
                                  selectedTaskMatchingItem.rawRequest
                                    ...
                                    selectedTaskMatchingItem.rawRequest.employer
                                      ?.companyDescription
                                    ? "text-indigo-655 font-bold block whitespace-pre-line"
                                    : ""
                                }
                              >
                                {selectedTaskMatchingItem.rawRequest
                        ...
                        <div className="bg-slate-50 border border-slate-200/80 p-3 rounded-xl space-y-1">
                          <p className="font-bold text-slate-700 pb-1 border-b border-slate-200 mb-1">
                            Thông tin ngân hàng đề xuất
                          </p>
                          <p>
                            <strong>Ngân hàng:</strong>{" "}
                            {selectedTaskMatchingItem.rawRequest.bankName ||
                              "Chưa cập nhật"}
                          </p>
                          <p>
                          ...
                          <p>
                            ...
                            {selectedTaskMatchingItem.rawRequest.branch ||
                              "Chưa cập nhật"}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="space-y-2 text-left">
                        <p className="text-body-sm font-bold text-[#141b2b]">
                          {selectedTaskMatchingItem.title}
                        </p>
                        <div className="bg-slate-50 border border-slate-200/80 p-3 rounded-xl text-xs text-slate-850 whitespace-pre-wrap leading-relaxed max-h-40 overflow-y-auto">
                          {selectedTaskMatchingItem.detail ||
                        ...
                        {selectedTaskMatchingItem.rawProject && (
                          <div className="bg-slate-50 border border-slate-200/80 p-3 rounded-xl text-xs space-y-1">
                            <p>
                              ...
                              {selectedTaskMatchingItem.rawProject.budget
                                ? selectedTaskMatchingItem.rawProject.budget.toLocaleString(
                                  "vi-VN",
                                ) + " VNĐ"
                                : "Liên hệ thỏa thuận"}
                            </p>
                            <p>
                              ...
                              {selectedTaskMatchingItem.rawProject.workType ||
                                "Chưa cập nhật"}
                            </p>
                            <p>
                              <strong>Kỹ năng yêu cầu:</strong>{" "}
                              {selectedTaskMatchingItem.rawProject
                                .requiredSkills || "Không yêu cầu"}
                            </p>
                          </div>
                        )}
                      </div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl w-full max-w-lg p-6 sm:p-7 shadow-2xl border border-[#e1e8fd] text-center animate-in fade-in zoom-in-95 duration-200">
            <div
              className={`mx-auto w-14 h-14 rounded-2xl mb-4 flex items-center justify-center ${confirmConfig.type === "danger"
            ...
            <h3 className="text-xl font-extrabold text-[#141b2b] mb-3">
              {confirmConfig.title}
            </h3>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button
                ...
                className={`flex-1 px-5 py-2.5 rounded-xl font-extrabold text-sm shadow-md transition-all text-white flex items-center justify-center gap-2 cursor-pointer ${confirmConfig.type === "danger"
      ...
      {showTransferRejectModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/45 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl w-full max-w-md p-6 shadow-xl border border-[#e1e8fd] animate-in fade-in zoom-in-95 duration-150 text-left">
            <h3 className="text-title-md font-extrabold text-[#141b2b] mb-4">
              Từ chối đơn điều chuyển
            </h3>
            <p className="text-body-sm text-[#3e4a3d] mb-3 font-semibold">
              Vui lòng nhập lý do từ chối đơn điều chuyển này (bắt buộc):
            </p>
            <textarea
              value={rejectReason}
          ...
          <div className="bg-white rounded-2xl w-full max-w-3xl shadow-2xl border border-[#e2eafc] flex flex-col max-h-[92vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-slate-400 font-semibold mt-0.5 text-left">
                    Mã số đơn:{" "}
                    <span className="text-[#006b2c] font-bold">
                      #REQ-{selectedTransferRequest.requestId}
                    </span>
                  </p>
                </div>
              </div>
            ...
            <div className="p-6 overflow-y-auto flex-1 custom-scrollbar space-y-6 text-left">
              ...
              <div>
                ...
                {/* trang thai khoi luong hien tai */}
                ...
                {(() => {
                  ...
                  return (
                    <div
                      className={`p-3.5 rounded-xl border mb-4 ${unfinishedCount > 0
                    ...
                    >
                      <div className="flex items-center gap-2 font-extrabold text-xs mb-1">
                        <AlertTriangle
                          className={`w-4 h-4 ${unfinishedCount > 0 ? "text-amber-600" : "text-emerald-600"}`}
                        />
                      ...
                      <p className="text-xs font-semibold leading-relaxed">
                        {unfinishedCount > 0 ? (
                          <>
                            ...
                            <strong className="text-amber-700 underline text-sm">
                              {unfinishedCount} công việc chưa hoàn thành
                            </strong>
                            . Yêu cầu staff cần bàn giao toàn bộ công việc trước
                            khi tiến hành điều chuyển phòng ban.
                          </>
                        ) : (
                          <>
                            Nhân viên đã hoàn thành tất cả công việc (
                            <strong>0 công việc dở dang</strong>). Đủ điều kiện
                            điều chuyển.
                          </>
                        )}
                      </p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-50/50 border border-slate-100 rounded-xl p-3 shadow-[0_2px_8px_rgba(0,0,0,0.01)]">
                    <span className="text-[10px] font-extrabold text-slate-400 tracking-wider block mb-0.5 uppercase">
                      Mã nhân viên
                    </span>
                    <span className="text-body-sm font-extrabold text-slate-700">
                      FP-{selectedTransferRequest.requestId}
                    </span>
                  </div>
                  <div className="bg-slate-50/50 border border-slate-100 rounded-xl p-3 shadow-[0_2px_8px_rgba(0,0,0,0.01)]">
                    <span className="text-[10px] font-extrabold text-slate-400 tracking-wider block mb-0.5 uppercase">
                    ...
                    <span className="text-body-sm font-extrabold text-slate-700">
                      {selectedTransferRequest.userDisplayName || "Nhân viên"}
                    </span>
                  </div>
                  <div className="bg-slate-50/50 border border-slate-100 rounded-xl p-3 shadow-[0_2px_8px_rgba(0,0,0,0.01)]">
                    <span className="text-[10px] font-extrabold text-slate-400 tracking-wider block mb-0.5 uppercase">
                    ...
                    <span className="text-body-sm font-extrabold text-emerald-700">
                      {selectedTransferRequest.fromDepartment || "Chưa rõ"}
                    </span>
                  </div>
                  <div className="bg-slate-50/50 border border-slate-100 rounded-xl p-3 shadow-[0_2px_8px_rgba(0,0,0,0.01)]">
                    <span className="text-[10px] font-extrabold text-slate-400 tracking-wider block mb-0.5 uppercase">
                    ...
                    <span className="text-body-sm font-extrabold text-slate-700">
                      {selectedTransferRequest.userEmail}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-blue-50/30 border border-blue-100/50 rounded-xl p-3 shadow-[0_2px_8px_rgba(0,0,0,0.01)] col-span-2">
                    <span className="text-[10px] font-extrabold text-blue-600 tracking-wider block mb-0.5 uppercase">
                      Phòng ban muốn chuyển đến
                    </span>
                    <span className="text-body-sm font-extrabold text-blue-700">
                      {selectedTransferRequest.toDepartment || "Chưa rõ"}
                    </span>
                  </div>
                  <div className="bg-slate-50/50 border border-slate-100 rounded-xl p-3 shadow-[0_2px_8px_rgba(0,0,0,0.01)]">
                    <span className="text-[10px] font-extrabold text-slate-400 tracking-wider block mb-0.5 uppercase">
                    ...
                    <span className="text-body-sm font-extrabold text-slate-700">
                      {parseReason(selectedTransferRequest.reason)
                        .desiredPosition || "Chưa cung cấp"}
                    </span>
                  </div>
                  <div className="bg-slate-50/50 border border-slate-100 rounded-xl p-3 shadow-[0_2px_8px_rgba(0,0,0,0.01)]">
                    <span className="text-[10px] font-extrabold text-slate-400 tracking-wider block mb-0.5 uppercase">
                    ...
                    <span className="text-body-sm font-extrabold text-slate-700">
                      {parseReason(selectedTransferRequest.reason).startDate ||
                        "Chưa cung cấp"}
                    </span>
                  </div>
              ...
              <div>
                ...
                <div className="bg-[#fcfdfe] border border-[#e2e8f0]/60 rounded-xl p-4 leading-relaxed text-body-sm text-slate-700 font-medium border-l-4 border-l-[#006b2c] shadow-[0_2px_10px_rgba(0,0,0,0.01)] whitespace-pre-wrap">
                  {parseReason(selectedTransferRequest.reason).reason ||
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div>
                    <span className="text-[10px] font-extrabold text-slate-400 tracking-wider block mb-1 uppercase">
                      Kỹ năng liên quan & Kinh nghiệm trước đây
                    </span>
                    <div className="bg-[#fcfdfe] border border-[#e2e8f0]/60 rounded-xl p-4 leading-relaxed text-body-sm text-slate-700 font-medium border-l-4 border-l-[#006b2c] shadow-[0_2px_10px_rgba(0,0,0,0.01)] whitespace-pre-wrap">
                      {parseReason(selectedTransferRequest.reason).skills ||
                        "Chưa cung cấp"}
                    </div>
                  </div>
                  <div>
                    <span className="text-[10px] font-extrabold text-slate-400 tracking-wider block mb-1 uppercase">
                      Thành tích nổi bật & Lý do phù hợp
                    </span>
                    <div className="bg-[#fcfdfe] border border-[#e2e8f0]/60 rounded-xl p-4 leading-relaxed text-body-sm text-slate-700 font-medium border-l-4 border-l-[#006b2c] shadow-[0_2px_10px_rgba(0,0,0,0.01)] whitespace-pre-wrap">
                      {parseReason(selectedTransferRequest.reason)
                        .achievements || "Chưa cung cấp"}
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-slate-50/40 border border-slate-100 rounded-xl p-3.5 flex items-center gap-3 shadow-[0_2px_8px_rgba(0,0,0,0.01)]">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-body-sm font-bold text-slate-700 truncate">
                      {parseReason(selectedTransferRequest.reason).attachment ||
                        "Không có tệp đính kèm"}
                    </p>
                    <p className="text-[10px] text-slate-400 font-semibold">
                      Định dạng hỗ trợ: PDF, DOCX, PNG, JPG
                    </p>
                  </div>
                </div>
              </div>
              ...
              {selectedTransferRequest.status !== "PENDING" &&
                selectedTransferRequest.decisionNote && (
                  <div className="p-4 bg-amber-50/70 border border-amber-100 rounded-xl space-y-1.5 border-l-4 border-l-amber-500">
                    <span className="text-[11px] font-bold text-amber-800 tracking-wider block uppercase">
                      {selectedTransferRequest.status === "APPROVED"
                        ? "Ghi chú phê duyệt từ Manager"
                        : "Lý do từ chối từ Manager"}
                    </span>
                    <p className="text-body-sm text-amber-900 font-bold leading-relaxed">
                      {selectedTransferRequest.decisionNote}
                    </p>
                  </div>
                )}
              ...
              {(selectedTransferRequest.handoverNotes ||
                selectedTransferRequest.notes ||
                selectedTransferRequest.status === "COMPLETED" ||
                selectedTransferRequest.handoverSubmitted) && (
                  <div className="p-5 bg-emerald-50/90 border border-emerald-200 rounded-2xl space-y-3">
                    ...
                    <div className="bg-white p-4 rounded-xl border border-emerald-100 text-sm text-slate-800 font-medium whitespace-pre-wrap leading-relaxed shadow-sm">
                      {selectedTransferRequest.handoverNotes ||
                        selectedTransferRequest.notes ||
                        "Staff đã hoàn tất xác nhận bàn giao công việc dở dang."}
                    </div>
                      ...
                      return (
                        <div className="mt-3 space-y-2">
                          <span className="text-xs font-extrabold text-emerald-900 uppercase tracking-wider block">
                            Danh sách các công việc thuộc hồ sơ bàn giao này (
                            {staffTasks.length} nhiệm vụ):
                          </span>
                            ...
                            <table className="w-full text-left text-xs">
                              ...
                              <tbody className="divide-y divide-emerald-100 font-medium text-slate-700">
                                {staffTasks.map((t) => (
                                  ...
                                  >
                                    <td className="px-3.5 py-2 font-bold text-[#006b2c]">
                                      {t.id}
                                    </td>
                                    ...
                                    <td className="px-3.5 py-2">
                                      <span
                                        className={`px-2 py-0.5 rounded text-[10px] font-extrabold border ${t.status === "Completed" ||
                                          t.status === "Manager đã ký duyệt"
                                          ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                                          : "bg-amber-50 text-amber-700 border-amber-200"
                                          }`}
                  ...
                  <button
                    onClick={() => {
                      ...
                      handleApproveTransferRequest(
                        selectedTransferRequest.requestId,
                        false,
                      );
                    }}
            ...
            <div className="p-6 overflow-y-auto flex-1 custom-scrollbar space-y-4">
              <div className="bg-rose-50 border border-rose-100 p-4 rounded-xl mb-4">
                <div className="flex justify-between mb-1">
                  <span className="text-xs font-bold text-rose-600 uppercase">
                    Ưu tiên: {selectedDispute.priority || "CAO"}
                  </span>
                ...
                <h3 className="text-body-lg font-bold text-[#141b2b]">
                  {selectedDispute.raw?.projectTitle || selectedDispute.title}
                </h3>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="bg-[#f7fff2] border border-[#d6f2c6] p-3 rounded-lg">
                  ...
                  <p className="font-bold text-[#141b2b]">
                    {selectedDispute.raw?.clientName || "SunGroup Corp"}
                  </p>
                </div>
                <div className="bg-blue-50 border border-blue-100 p-3 rounded-lg">
                  <p className="text-xs text-[#3e4a3d] mb-1">Bên Freelancer</p>
                  <p className="font-bold text-[#141b2b]">
                    {selectedDispute.raw?.freelancerName || "Tran Thu Ha"}
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-title-lg text-rose-600 font-extrabold">
                  {(selectedDispute.raw?.amount || 8000000)?.toLocaleString(
                    "vi-VN",
                  )}{" "}
                  VND
                </p>
              ...
              <div>
                ...
                <div className="bg-[#f1f4f0] p-3 rounded-lg text-sm text-[#141b2b]">
                  {selectedDispute.raw?.reason ||
                    "Sản phẩm thiết kế không đúng mô tả thiết kế ban đầu, quá nhiều lỗi"}
                </div>
              </div>
            </div>
              ...
              {showAssignStaffDrawer && (
                <div className="bg-white p-3.5 rounded-xl border border-indigo-200 shadow-sm flex flex-col sm:flex-row items-center gap-3 animate-in fade-in zoom-in-95 duration-150">
                  <div className="flex-1 w-full">
                    ...
                    >
                      <option value="">
                        -- Chọn nhân viên thuộc phòng {
                          currentDeptCode === "DIS" ? "Tranh chấp" :
                          currentDeptCode === "MOD" ? "Kiểm duyệt" :
                          currentDeptCode === "FIN" ? "Tài chính" :
                          currentDeptCode === "CS" ? "Hỗ trợ khách hàng" :
                          currentDeptCode === "IT" ? "Kỹ thuật" : "Ban quản lý"
                        } --
                      </option>
                      {(() => {
          ...
          <div className="bg-white rounded-xl w-full max-w-md shadow-xl border border-[#e1e8fd] flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="p-6 overflow-y-auto flex-1 custom-scrollbar space-y-4 text-sm">
              <div className="flex justify-between items-center mb-1">
                ...
                <span
                  className={`px-2 py-0.5 rounded text-[10px] font-bold ${selectedWithdrawal.statusRaw === "PENDING"
                    ? "bg-amber-100 text-amber-800"
      ...
      {showTransferRequestModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/45 backdrop-blur-sm px-4">
          <div className="bg-white rounded-xl w-full max-w-md shadow-xl border border-[#e1e8fd] flex flex-col animate-in fade-in zoom-in-95 duration-150">
            ...
            >
              <div>
                <label className="block text-body-sm text-[#3e4a3d] font-bold mb-1">
                  Họ tên & Email
                </label>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setTransferRequestTargetDeptId(e.target.value)
                  }
                >
              ...
              <div>
                <label className="block text-body-sm text-[#3e4a3d] font-bold mb-2">
                  Lý do điều chuyển
                </label>
                <textarea
                  required
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={
                    isSubmittingTransferRequest || !transferRequestTargetDeptId
                  }
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/45 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl border border-[#e1e8fd] flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="px-6 py-4 border-t border-[#e1e8fd] bg-slate-50 flex items-center justify-between rounded-b-2xl">
              ...
              <span
                className={`text-xs font-bold px-3 py-1.5 rounded-lg ${selectedModerationItem.status === "Pending"
                  ? "bg-amber-100 text-amber-800"
                  : "bg-[#f7fff2] text-[#006b2c]"
                  }`}
              >
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 overflow-y-auto space-y-5">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span
                  className={`px-3 py-1 text-xs font-bold border rounded-md ${selectedReport.status === "Đã chuyển cấp" ||
                ...
                selectedReport.status === "ESCALATED") && (
                  <div className="flex items-center gap-2">
                    {(() => {
                      const isAssigned = tasks.some(
                        (t) =>
                          ...
                          t.status !== "Rejected" &&
                          String(t.referenceId) ===
                          String(selectedReport.idRaw || selectedReport.id) &&
                          t.taskType === "REPORT_RESOLUTION",
                      );
                      return isAssigned ? (
                        ...
                        <button
                          onClick={() => {
                            ...
                            setCreateForm({
                              ...
                              requiredDepartments:
                                activeDeptCode === "ALL" ? "MOD" : activeDeptCode,
                              assignedToEmail: "",
                            });
                            setShowCreateModal(true);
                          }}
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleManagerResolveReport(selectedReport, "REJECTED")
                      }
                    ...
                    <button
                      disabled={reportActionLoading}
                      onClick={() =>
                        handleManagerResolveReport(selectedReport, "RESOLVED")
                      }
