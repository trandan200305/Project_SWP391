      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-16 border-b border-slate-200 bg-white flex items-center justify-between px-6 shrink-0 z-10 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <span className={`hidden sm:inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold ${isConnected ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200'
              }`}>
        ...
        <div className="flex-1 flex overflow-hidden">
                    ...
                    <div className="flex gap-1.5 mt-4 items-center justify-between">
                      ...
                      >
                        ...
                        {activeTickets.length > 0 && (
                          <span className={`w-4 h-4 text-[9px] font-black rounded-full flex items-center justify-center ${activeTab === 'active' ? 'bg-white/25' : 'bg-blue-500 text-white'
                      ...
                      >
                        ...
                        {pendingTickets.length > 0 && (
                          <span className={`w-4 h-4 text-[9px] font-black rounded-full flex items-center justify-center ${activeTab === 'pending' ? 'bg-white/25 text-white' : 'bg-amber-500 text-white'
                      ...
                      >
                        ...
                        {blockedTickets.length > 0 && (
                          <span className={`w-4 h-4 text-[9px] font-black rounded-full flex items-center justify-center ${activeTab === 'blocked' ? 'bg-white/25 text-white' : 'bg-slate-500 text-white'
                      ...
                      >
                        ...
                        {deletedTickets.length > 0 && (
                          <span className={`w-4 h-4 text-[9px] font-black rounded-full flex items-center justify-center ${activeTab === 'deleted' ? 'bg-white/25 text-white' : 'bg-rose-500 text-white'
                    ...
                    <div className="flex gap-2.5 mt-4 items-center justify-start">
                      ...
                      >
                        ...
                        {activeDirectChats.length > 0 && (
                          <span className={`w-4 h-4 text-[9px] font-black rounded-full flex items-center justify-center ${activeDirectTab === 'active' ? 'bg-white/25' : 'bg-blue-500 text-white'
                      ...
                      >
                        ...
                        {blockedDirectChats.length > 0 && (
                          <span className={`w-4 h-4 text-[9px] font-black rounded-full flex items-center justify-center ${activeDirectTab === 'blocked' ? 'bg-white/25' : 'bg-slate-500 text-white'
                      ...
                      >
                        ...
                        {deletedDirectChats.length > 0 && (
                          <span className={`w-4 h-4 text-[9px] font-black rounded-full flex items-center justify-center ${activeDirectTab === 'deleted' ? 'bg-white/25' : 'bg-rose-500 text-white'
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-1.5 mb-1">
                                    <span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded border uppercase tracking-wider ${ticket.sender_role === 'EMPLOYER' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-blue-50 text-blue-600 border-blue-100'
                      ...
                      {filteredDirectChats.map(chat => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex justify-between items-center mb-0.5">
                                <h4 className={`font-extrabold truncate pr-2 text-sm ${chat.isDeleted ? 'line-through text-slate-500' : (chat.unreadCount > 0 ? 'text-blue-900' : 'text-slate-900')
                                  }`}>
                                  {chat.partnerName}
                                </h4>
                                <span className={`text-[10px] font-bold whitespace-nowrap ${chat.unreadCount > 0 ? 'text-blue-600' : 'text-slate-400'
                                  }`}>
                                  {formattedTime}
                                </span>
                              </div>
                              <div className="flex items-center gap-1.5 mb-1">
                                <span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded border uppercase tracking-wider ${chat.partnerRole === 'EMPLOYER' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-blue-50 text-blue-600 border-blue-100'
                ...
                <div className="flex flex-col h-full bg-white">
                  ...
                  {isProfileLoading ? (
                    ...
                    <div className="flex flex-col flex-1 overflow-y-auto p-6 items-center">
                      ...
                      <span className={`text-[10px] font-extrabold px-2.5 py-1 rounded-md border uppercase tracking-widest mb-6 ${selectedProfile.role === 'EMPLOYER'
          ...
          <div className={`flex-1 flex h-full overflow-hidden ${(!activeTicket && !activeDirectChat) && 'hidden md:flex'
            }`}>
                            ...
                            <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-full`}>
                              {!isMe && (
                                <div className="flex items-center gap-1.5 mb-1.5 ml-1 select-none">
                                  ...
                                  <span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded-md uppercase tracking-wider ${msg.senderRole?.toUpperCase() === 'ADMIN'
                              ...
                              {msg.messageText && msg.messageText.trim() !== '' && !(msg.attachments && msg.attachments.length > 0 && (msg.messageText === '[Hình ảnh]' || msg.messageText === '[Tệp đính kèm]')) && (
                                <div className={`p-3.5 rounded-2xl text-[14px] leading-relaxed shadow-sm font-medium transition-all duration-300 ${isMe
                                  ...
                                  {msg.attachments.map((att, attIdx) => {
                                    ...
                                    if (isImg) {
                                      ...
                                      return (
                                        ...
                                        >
                                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${isMe ? 'bg-blue-500/25 text-white' : 'bg-slate-100 text-slate-500'
                                            }`}>
                                          ...
                                          <div className={`p-1.5 rounded-lg shrink-0 ${isMe ? 'hover:bg-blue-600/30 text-white' : 'hover:bg-slate-100 text-slate-500'
                                            }`}>
                              ...
                              <p className={`text-[9px] font-bold text-slate-400 mt-1.5 flex items-center gap-1 ${isMe ? 'justify-end mr-1' : 'ml-1'
                                }`}>
                      ...
                      {(() => {
                        ...
                        return (
                          <button
                            type="submit"
                            disabled={!canSend}
                            className={`p-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl shadow-md shadow-blue-500/20 flex items-center justify-center transition-all ${!canSend ? 'opacity-50 cursor-not-allowed bg-slate-300 shadow-none' : ''
                              }`}
                          >
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className={`flex flex-col ${isMine ? 'items-end' : 'items-start'} max-w-full`}>
                              <div className={`p-3.5 rounded-2xl text-[14px] leading-relaxed shadow-sm font-medium transition-all duration-300 ${isMine
                                ? 'bg-gradient-to-br from-blue-600 to-indigo-600 text-white rounded-br-none border border-blue-500/20 shadow-md shadow-blue-500/10'
                                : 'bg-white text-slate-800 border border-slate-200/60 rounded-bl-none shadow-sm'
                  ...
                  {activeDirectChat.isBlocked ? (
                    ...
                    >
                      ...
                      <button
                        ...
                        className={`p-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl shadow-md shadow-blue-500/20 flex items-center justify-center transition-all ${inputText.trim() === '' ? 'opacity-50 cursor-not-allowed bg-slate-300 shadow-none' : ''
                          }`}
                      >
                        <Send className="w-5 h-5 shrink-0" />
                      </button>
            ...
            {showUserInfo && activeTicket && user?.role === 'ADMIN' && (
              <div className="w-80 border-l border-slate-200 bg-white flex flex-col h-full shrink-0 overflow-y-auto">
                <div className="p-6 border-b border-slate-100 flex flex-col items-center">
                  ...
                  <span className={`text-[10px] font-extrabold px-2 py-1 rounded border uppercase tracking-wider ${activeTicket.sender_role === 'EMPLOYER' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-blue-50 text-blue-600 border-blue-100'
            ...
            {showUserInfo && activeDirectChat && navSection === 'direct_chats' && (
              <div className="w-80 border-l border-slate-200 bg-white flex flex-col h-full shrink-0 overflow-y-auto">
                <div className="p-6 border-b border-slate-100 flex flex-col items-center">
                  ...
                  <span className={`text-[10px] font-extrabold px-2 py-1 rounded border uppercase tracking-wider ${activeDirectChat.partnerRole === 'EMPLOYER' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-blue-50 text-blue-600 border-blue-100'
        ...
        {showConfirmModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
            <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl animate-fade-in text-center">
              <div className={`mx-auto w-12 h-12 rounded-full mb-4 flex items-center justify-center ${confirmConfig.type === 'danger' ? 'bg-rose-100 text-rose-600' :
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-xl font-bold text-white shadow-md transition-all ${confirmConfig.type === 'danger' ? 'bg-rose-600 hover:bg-rose-700 shadow-rose-600/20' :
