...
export default function Messenger({ user, onNavigateHome, initialPartner }) {
  ...
  const isSupportAgentRole = (role) =>
    ["ADMIN", "STAFF"].includes(normalizeRole(role));
  ...
  useEffect(() => {
    if (sidebarWidth < 160 && navSection === "user_profile") {
      setNavSection("chat");
    }
  }, [sidebarWidth, navSection]);

  useEffect(() => {
    const handleMouseMove = (e) => {
      if (!isResizingRef.current) return;
      const container = document.getElementById("messenger-sidebar-container");
      if (container) {
        ...
        client.subscribe("/topic/admin", (message) => {
          ...
          if (receivedMessage.senderRole === "SYSTEM") {
            if (
              ...
              receivedMessage.messageText.startsWith("CLAIM_UPDATE:")
            ) {
              const targetTicketId = receivedMessage.ticketId;
              const staffId = parseInt(
                receivedMessage.messageText.split(":")[1],
              );
              const staffName = receivedMessage.senderName;
              const staffAvatar = receivedMessage.senderAvatar;
              ...
              setActiveTicket((prev) => {
                if (
                  prev &&
                  (prev.ticket_id === targetTicketId ||
                    prev.ticketId === targetTicketId)
                ) {
                  return {
                    ...prev,
                    assigned_staff_id: staffId,
          ...
          setTickets((prevTickets) => {
            const ticketIndex = prevTickets.findIndex(
              (t) => t.ticket_id === receivedMessage.ticketId,
            );
            ...
            if (ticketIndex !== -1) {
              ...
              const isAdminRealReply =
                (receivedMessage.senderRole === "ADMIN" ||
                  receivedMessage.senderRole === "STAFF") &&
                receivedMessage.messageText &&
                !receivedMessage.messageText.startsWith(
                  "\uD83D\uDC4B Xin ch\u00e0o",
                );
              updatedTickets[ticketIndex] = {
                ...
                has_admin_replied:
                  updatedTickets[ticketIndex].has_admin_replied ||
                  isAdminRealReply,
              };
          ...
          if (activeTicketIdRef.current === receivedMessage.ticketId) {
            setMessages((prev) => {
              if (
                prev.some((msg) => msg.messageId === receivedMessage.messageId)
              )
                return prev;
  ...
  const getOrCreateUserTicket = async () => {
    setIsLoading(true);
    try {
      const ticketData = await messengerApi.getOrCreateTicket(
        user.id,
        user.role,
      );
      ...
      if (stompClientRef.current && stompClientRef.current.connected) {
        stompClientRef.current.publish({
          ...
          body: JSON.stringify({
            ticketId: ticketId,
            readerRole: user?.role?.toUpperCase(),
          }),
        });
      }
    } catch (err) {
  ...
  // 3.2 load lịch sử chat của ticket đã chọn
  ...
  const subscribeToTicket = (ticketId) => {
    ...
    // nhận tin nhắn tới
    ticketSubscriptionRef.current = stompClientRef.current.subscribe(
      ...
      (message) => {
        ...
        if (receivedMessage.senderRole === "SYSTEM") {
          if (
            receivedMessage.messageText &&
            receivedMessage.messageText.startsWith("BLOCK_UPDATE:")
          ) {
            ...
            setActiveTicket((prev) => {
              if (
                prev &&
                (prev.ticket_id === ticketId || prev.ticketId === ticketId)
              ) {
          ...
          if (
            ...
            const staffId = parseInt(receivedMessage.messageText.split(":")[1]);
            const staffName = receivedMessage.senderName;
            const staffAvatar = receivedMessage.senderAvatar;
  ...
  // 3.4 khi admin/staff chọn ticket thì load lịch sử và subscribe realtime
  ...
  const subscribeToDirectChat = (chatId) => {
    ...
    directChatSubscriptionRef.current = stompClientRef.current.subscribe(
  ...
  const handleFileChange = async (e, isImageOnly = false) => {
    ...
    for (const file of files) {
      ...
      try {
        const data = await messengerApi.uploadFile(formData); //upload file/ảnh
        if (data.success) {
          newAttachments.push({
  ...
  // 5.2 xóa cuộc trò chuyện
  const handleDeleteTicket = async () => {
    if (!activeTicket) return;
    setConfirmConfig({
      title: "Xóa cuộc trò chuyện",
      message:
        "Bạn có chắc chắn muốn xóa cuộc trò chuyện này? Người dùng sẽ không biết cuộc trò chuyện đã bị xóa.",
      ...
      onConfirm: async () => {
        try {
          ...
          setTickets((prev) =>
            prev.filter((t) => t.ticket_id !== activeTicket.ticket_id),
          );
          setActiveTicket(null);
          setShowUserInfo(false);
          fetchDeletedTickets();
  ...
  const handleClaimTicket = async () => {
    ...
    try {
      setIsLoading(true);
      //gọi API
      await messengerApi.claimTicket(ticketId, user.id);
      ...
      alert("Không thể tiếp nhận ticket hỗ trợ này. Vui lòng thử lại sau.");
    } finally {
      setIsLoading(false);
    }
  };

  // 6. Gửi tin nhắn đi support/direct
  const handleSendMessage = (e) => {
    ...
    if (
      ...
      alert(
        isAgent
          ? "Tài khoản này đang bị chặn, bạn không thể gửi tin nhắn."
          : "Bạn đã bị chặn gửi tin nhắn trong cuộc hội thoại này.",
      );
      return;
    }
    ...
    // Chuẩn bị payload và gửi qua backend
    const payload = {
      ticketId: ticketId,
      senderId: user.id,
  ...
  const handleDeleteDirectChat = async (chatId) => {
    setConfirmConfig({
      title: "Xóa cuộc trò chuyện",
      message:
        "Bạn có chắc chắn muốn xóa cuộc trò chuyện này? Đối tác của bạn vẫn sẽ nhìn thấy cuộc trò chuyện từ phía họ.",
      confirmText: "Xóa hội thoại",
      cancelText: "Hủy",
      type: "danger",
      onConfirm: async () => {
        try {
          ...
          setDirectChats((prev) =>
            prev.map((c) =>
              c.chatId === chatId ? { ...c, isDeleted: true } : c,
            ),
          );
          setActiveDirectChat(null);
          setShowConfirmModal(false);
    ...
    setConfirmConfig({
      ...
      onConfirm: async () => {
        try {
          ...
          setDirectChats((prev) =>
            prev.map((c) =>
              c.chatId === chatId ? { ...c, isDeleted: false } : c,
            ),
          );
          setActiveDirectChat(null);
          setShowConfirmModal(false);
  ...
  const handleBlockDirectChat = async (chatId, days = -1) => {
    ...
    if (days === -1) {
      ...
      confirmMsg =
        "Bạn có chắc chắn muốn chặn người dùng này vĩnh viễn? Cả hai sẽ không thể gửi tin nhắn cho nhau.";
      confirmBtn = "Chặn vĩnh viễn";
      typeBtn = "danger";
    } else {
      ...
      onConfirm: async () => {
        try {
          ...
          setActiveDirectChat((prev) =>
            prev ? { ...prev, isBlocked: true, isBlockedByMe: true } : null,
          );
          setShowConfirmModal(false);
        } catch (err) {
  ...
  const handleUnblockDirectChat = async (chatId) => {
    setConfirmConfig({
      ...
      message:
        "Bạn có chắc chắn muốn bỏ chặn người dùng này để tiếp tục gửi tin nhắn?",
      confirmText: "Bỏ chặn",
      cancelText: "Hủy",
      type: "success",
      onConfirm: async () => {
  ...
  const handleStartChatWithUser = async (clickedUser, role) => {
    ...
    try {
      ...
      const activeT = {
        ...
        sender_avatar:
          clickedUser.avatarUrl ||
          `https://ui-avatars.com/api/?name=${clickedUser.name || "User"}&background=3b82f6&color=fff`,
  ...
  const blockedTickets = tickets.filter(
    (t) =>
      t.blocked_until &&
      new Date(t.blocked_until) > new Date() &&
      matchesSearch(t),
  );
  const pendingTickets = tickets.filter(
    (t) =>
      !t.assigned_staff_id &&
      matchesSearch(t) &&
      !(t.blocked_until && new Date(t.blocked_until) > new Date()),
  );
  ...
  const activeDirectChats = (directChats || []).filter(
    (c) => !c.isDeleted && !c.isBlocked && matchesDirectSearch(c),
  );
  const blockedDirectChats = (directChats || []).filter(
    (c) => c.isBlocked && matchesDirectSearch(c),
  );
  const deletedDirectChats = (directChats || []).filter(
    (c) => c.isDeleted && matchesDirectSearch(c),
  );
    ...
    <div className="flex h-screen bg-slate-50/50 text-slate-800 font-sans overflow-hidden">
      <style>{`
        ...
        #messenger-sidebar-container .flex-1.min-w-0.opacity-80 {
          opacity: ${textOpacity};
          display: ${sidebarWidth < 140 ? "none !important" : "block"};
          transition: opacity 0.1s ease;
        }
        #messenger-sidebar-container div[onClick].flex {
          padding: ${sidebarWidth < 140 ? "12px 8px !important" : "16px !important"};
          justify-content: ${sidebarWidth < 140 ? "center !important" : "flex-start !important"};
          gap: ${sidebarWidth < 140 ? "0 !important" : "14px !important"};
        }
        #messenger-sidebar-container > div:first-child {
          display: ${sidebarWidth < 140 ? "none !important" : "block"};
          opacity: ${textOpacity};
          transition: opacity 0.1s ease;
        }
        ...
        <div>
          ...
          >
            ...
            <div className="overflow-hidden">
              <h1 className="font-extrabold text-lg leading-tight tracking-tight text-white truncate">
                LancerPro
              </h1>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-extrabold tracking-tight text-slate-900">
                {isAgent ? "Support Console" : "Kênh Hỗ Trợ Kỹ Thuật"}
              </h2>
            </div>
            <span
        ...
        <div className="flex-1 flex overflow-hidden">
                ...
                <>
                      ...
                      {activeTickets.length > 0 ? (
                        <div>
                          {activeTickets.map((ticket) => {
                            ...
                            const date = ticket.last_message_at
                              ...
                              >
                                <div className="relative shrink-0">
                                  <img
                                    src={
                                      ...
                                      `https://ui-avatars.com/api/?name=${ticket.sender_name || "Client"}&background=eff6ff&color=3b82f6`
                                    }
                                  ...
                                  {(() => {
                                    ...
                                    const lastLoginStr =
                                      ticket.sender_last_login;
                                    const lastLoginTime = lastLoginStr
                                      ? new Date(lastLoginStr).getTime()
                                      : 0;
                                    const isOnline =
                                      lastLoginTime > 0 &&
                                    ...
                                    if (isLocked) {
                                      ...
                                      return (
                                        <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-rose-500 rounded-full border-2 border-white"></div>
                                      );
                                    } else if (isOnline) {
                                      return (
                                      ...
                                      return (
                                        <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-slate-400 rounded-full border-2 border-white"></div>
                                      );
                                    }
                                  })()}
                                </div>
                                <div className="flex-1 min-w-0">
                                  <div className="flex justify-between items-center mb-0.5">
                                    <h4 className="font-extrabold text-slate-900 truncate pr-2 text-sm">
                                      {ticket.sender_name}
                                    </h4>
                                  ...
                                  <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                                    ...
                                    <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded border uppercase tracking-wider bg-emerald-50 text-emerald-600 border-emerald-100">
                                      Đang xử lý
                                    </span>
                                    {ticket.staff_name && (
                                      <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded border bg-slate-50 text-slate-500 border-slate-200">
                                        Staff: {ticket.staff_name}
                                      </span>
                                    )}
                                  </div>
                                  <p
                                    className={`text-xs truncate font-medium ${isSelected ? "text-slate-800" : "text-slate-500"}`}
                                  >
                      ...
                      <>
                        <div className="px-4 py-2 bg-amber-50 border-b border-amber-100 flex items-center gap-2">
                          ...
                          <p className="text-[10px] font-extrabold text-amber-700 uppercase tracking-wider">
                            Đang chờ phản hồi ({pendingTickets.length})
                          </p>
                        </div>
                            ...
                            >
                              <div className="relative shrink-0">
                                <img
                                  src={
                                    ticket.sender_avatar ||
                                    `https://ui-avatars.com/api/?name=${ticket.sender_name || "Client"}&background=fff7ed&color=d97706`
                                  }
                                  alt={ticket.sender_name}
                                ...
                                {(() => {
                                  ...
                                  const isOnline =
                                    lastLoginTime > 0 &&
                                    Date.now() - lastLoginTime < 5 * 60 * 1000;
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-center mb-0.5">
                                  <h4 className="font-extrabold text-slate-800 truncate pr-2 text-sm">
                                    {ticket.sender_name}
                                  </h4>
                                ...
                                <div className="flex items-center gap-1.5 mb-1">
                                  ...
                                  <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded border uppercase tracking-wider bg-amber-50 text-amber-600 border-amber-200">
                                    Chờ xử lý
                                  </span>
                                </div>
                          ...
                          >
                            <div className="relative shrink-0">
                              <img
                                src={
                                  ticket.sender_avatar ||
                                  `https://ui-avatars.com/api/?name=${ticket.sender_name || "Client"}&background=f1f5f9&color=64748b`
                                }
                                alt={ticket.sender_name}
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-1.5 mb-1">
                                ...
                                <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded border uppercase tracking-wider bg-slate-100 text-slate-600 border-slate-200">
                                  Đã chặn
                                </span>
                              </div>
                              <p className="text-xs truncate font-medium text-slate-500">
                  ...
                  {activeTab === "blocked" && blockedTickets.length === 0 && (
                    <div className="p-8 text-center text-slate-400">
                      <AlertCircle className="w-8 h-8 mx-auto mb-2 text-slate-300" />
                      <p className="text-xs font-semibold">
                        Không có cuộc trò chuyện nào bị chặn
                      </p>
                    </div>
                  )}
                          ...
                          >
                            <div className="relative shrink-0">
                              <img
                                src={
                                  ticket.sender_avatar ||
                                  `https://ui-avatars.com/api/?name=${ticket.sender_name || "Client"}&background=ffe4e6&color=e11d48`
                                }
                                alt={ticket.sender_name}
                            ...
                            <div className="flex-1 min-w-0 opacity-80">
                              <div className="flex justify-between items-center mb-0.5">
                                <h4 className="font-extrabold text-slate-800 truncate pr-2 text-sm line-through decoration-slate-400">
                                  {ticket.sender_name}
                                </h4>
                              ...
                              <div className="flex items-center gap-1.5 mb-1">
                                ...
                                <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded border uppercase tracking-wider bg-rose-50 text-rose-600 border-rose-200">
                                  Đã xoá
                                </span>
                              </div>
                              <p className="text-xs truncate font-medium text-slate-500">
                  ...
                  {activeTab === "deleted" && deletedTickets.length === 0 && (
                    <div className="p-8 text-center text-slate-400">
                      <AlertCircle className="w-8 h-8 mx-auto mb-2 text-slate-300" />
                      <p className="text-xs font-semibold">
                        Không có cuộc trò chuyện nào trong thùng rác
                      </p>
                    </div>
                  )}
                  ...
                  {filteredDirectChats.length > 0 ? (
                    <div>
                      {filteredDirectChats.map((chat) => {
                        const isSelected =
                          activeDirectChat?.chatId === chat.chatId;
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex justify-between items-center">
                                <p
                                  className={`text-xs truncate font-medium ${chat.unreadCount > 0 ? "text-blue-700 font-bold" : "text-slate-500"}`}
                                >
                                  {chat.isDeleted
                                    ? "Tin nhắn đã bị ẩn"
                                    : chat.lastMessage ||
                      ...
                      {filteredSystemUsers.map((u) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-slate-400 truncate font-semibold mb-1">
                              {u.email}
                            </p>
                            <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded border uppercase tracking-wider bg-blue-50 text-blue-600 border-blue-100">
                              FREELANCER
                            </span>
                          </div>
                        </div>
                      ...
                      {filteredSystemUsers.map((u) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded border uppercase tracking-wider bg-pink-50 text-pink-600 border-pink-100">
                              EMPLOYER
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="relative mb-4 group">
                          <img
                            src={
                              ...
                              `https://ui-avatars.com/api/?name=${profileDetails.name || selectedProfile.name}&background=${selectedProfile.role === "EMPLOYER" ? "fdf2f8" : "eff6ff"}&color=${selectedProfile.role === "EMPLOYER" ? "db2777" : "3b82f6"}&size=128`
                            }
                          ...
                          />
                          <div
                            className={`absolute bottom-2 right-2 w-5 h-5 rounded-full border-2 border-white ${profileDetails.status === "OFFLINE" ? "bg-slate-400" : "bg-emerald-500"}`}
                          ></div>
                        </div>

                        <h2 className="text-xl font-extrabold text-slate-900 mb-1">
                          {profileDetails.name || selectedProfile.name}
                        </h2>
                        <p className="text-sm text-slate-500 mb-2 font-medium">
                          {profileDetails.email || selectedProfile.email}
                        </p>
                        ...
                        <div className="w-full bg-slate-50 rounded-2xl p-5 mb-6 border border-slate-100 shadow-sm">
                          ...
                          <p className="text-sm text-slate-600 leading-relaxed">
                            {profileDetails.bio ||
                              ...
                              "Chưa có thông tin giới thiệu chi tiết về người dùng này."}
                          </p>
                        </div>
          ...
          >
              ...
              {navSection === "chat" && activeTicket ? (
                      ...
                      <div>
                        ...
                        {!isAgent ? (
                          <p className="text-[10px] text-emerald-600 font-bold flex items-center gap-1">
                            {activeTicket.staff_name
                              ? `Đang hỗ trợ bởi ${activeTicket.staff_name}`
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50/50">
                    {isLoading ? (
                      ...
                      messages.map((msg, index) => {
                        const isMe = isOwnSupportMessage(msg);
                          ...
                          >
                            {!isMe && (
                              <img
                                src={
                                  msg.senderAvatar ||
                                  `https://ui-avatars.com/api/?name=${msg.senderName || "Staff"}&background=3b82f6&color=fff`
                                }
                                alt={msg.senderName}
                            ...
                            >
                                      ...
                                      if (isImg) {
                                        return (
                                          ...
                                          >
                                            ...
                                            />
                                            <div
                                              className={`absolute inset-0 transition-all duration-300 flex items-center justify-center ${activeTab === "deleted" ? "bg-black/10 pointer-events-none" : "bg-black/0 group-hover:bg-black/10"}`}
                                            >
                                              <span
                                                className={`text-white text-xs font-bold bg-black/60 px-3 py-1.5 rounded-full transition-all duration-300 ${activeTab === "deleted" ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
                                              >
                                                {activeTab === "deleted"
                                                  ? "Nội dung đã bị xóa"
                                                  : "Xem ảnh gốc"}
                                        ...
                                        return (
                                          ...
                                          >
                                            ...
                                            <div className="min-w-0 flex-1">
                                              ...
                                              <p
                                                className={`text-[10px] ${isMe ? "text-blue-200/80" : "text-slate-400"} font-medium`}
                                              >
                                                {att.fileSize
                              ...
                              >
                                ...
                                {isMe &&
                                  (msg.isRead || msg.read ? (
                                    <span className="ml-1 text-[9px] text-blue-500 font-bold">
                                      Đã xem
                                    </span>
                                  ) : (
                  ...
                  new Date(activeTicket.blocked_until) > new Date() ? (
                    <div className="flex items-center justify-center p-4 bg-slate-100 border-t border-slate-200 h-[76px]">
                      ...
                      <span className="text-sm font-semibold text-slate-600">
                        {isAgent
                          ? "Tài khoản này đang bị chặn gửi tin nhắn"
                          : "Bạn đã bị chặn gửi tin nhắn tạm thời"}
                      </span>
                    </div>
                  ) : (
                    (() => {
                      const isAssignedToOther =
                        activeTicket?.assigned_staff_id &&
                        activeTicket?.assigned_staff_id !== user?.id;
                      ...
                      if (isAgent) {
                        if (isUnassigned) {
                          ...
                          if (user?.role === "STAFF") {
                            bannerText =
                              "Hội thoại hỗ trợ này chưa được tiếp nhận.";
                            bannerType = "claim";
                            lockInput = true;
                          } else {
                            bannerText =
                              "Hội thoại này chưa được tiếp nhận (Admin có quyền gửi tin nhắn).";
                            bannerType = "info";
                            lockInput = false;
                          }
                          ...
                          showBanner = true;
                          const handlerName =
                            activeTicket?.staff_name || "nhân viên khác";
                          if (user?.role === "STAFF") {
                            bannerText = `Hội thoại này đang được xử lý bởi ${handlerName}. Bạn chỉ có quyền xem.`;
                            bannerType = "view_only";
                            lockInput = true;
                          } else {
                            bannerText = `Hội thoại này đang được xử lý bởi ${handlerName} (Admin có quyền gửi tin nhắn).`;
                            bannerType = "info";
                            lockInput = false;
                          }
                        }
                      ...
                      if (showBanner && lockInput) {
                        return (
                          <div className="p-4 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4 shrink-0 animate-fade-in">
                            ...
                            {bannerType === "claim" && (
                              <button
                                ...
                                className="w-full sm:w-auto px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold rounded-xl shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2"
                              >
                      ...
                      return (
                        <>
                          {showBanner && (
                            <div className="px-6 py-2.5 bg-blue-50/50 border-t border-slate-200 flex items-center gap-2 shrink-0">
                          ...
                          >
                            ...
                            <input
                              ...
                              placeholder={
                                isAgent
                                  ? "Trả lời yêu cầu kỹ thuật của khách hàng..."
                                  : "Nhập tin nhắn gửi kỹ thuật hỗ trợ..."
                              }
                              value={inputText}
                ...
                <>
                  <div className="h-16 px-6 border-b border-slate-200 bg-white flex items-center justify-between shrink-0">
                    ...
                    >
                      ...
                      <img
                        src={
                          activeDirectChat.partnerAvatar ||
                          `https://ui-avatars.com/api/?name=${activeDirectChat.partnerName || "User"}&background=eff6ff&color=3b82f6`
                        }
                        alt="Partner"
                        className="w-10 h-10 rounded-xl object-cover border border-slate-200"
                      />
                      ...
                      <ChevronRight
                        className={`w-4 h-4 text-slate-300 transition-transform duration-200 ${showUserInfo ? "rotate-180" : ""}`}
                      />
                    </div>
                    ...
                    {messages.length === 0 ? (
                      ...
                      messages.map((msg, index) => {
                        const isMine =
                          ...
                          msg.senderRole === user?.role?.toUpperCase();
                        return (
                          <div
                            key={index}
                            className={`flex items-start gap-2.5 max-w-[80%] ${
                          ...
                          >
                            {!isMine && (
                              <img
                                src={
                                  activeDirectChat?.partnerAvatar ||
                                  `https://ui-avatars.com/api/?name=${activeDirectChat?.partnerName || "User"}&background=3b82f6&color=fff`
                                }
                  ...
                  {activeDirectChat.isBlocked ? (
                    ...
                    <div className="flex items-center justify-center p-4 bg-slate-100 border-t border-slate-200 h-[76px] shrink-0">
                      ...
                      <span className="text-sm font-semibold text-slate-600">
                        Cuộc trò chuyện đang nằm trong thùng rác. Hãy khôi phục
                ...
                <div className="hidden md:flex flex-1 flex-col items-center justify-center p-8 text-center bg-slate-50/50">
                  ...
                  <p className="text-xs text-slate-500 font-semibold max-w-xs leading-relaxed">
                    Chọn một trong các cuộc hội thoại ở danh sách bên trái để
                    phản hồi và giải quyết yêu cầu kỹ thuật của khách hàng trực
                    tuyến.
                  </p>
                </div>
              )}
            ...
            {showUserInfo && activeTicket && isAgent && (
                ...
                <div className="p-6 border-b border-slate-100 flex flex-col items-center">
                  <div className="relative mb-4">
                    <img
                      src={
                        activeTicket.sender_avatar ||
                        `https://ui-avatars.com/api/?name=${activeTicket.sender_name || "Client"}&background=eff6ff&color=3b82f6`
                      }
                      alt="User avatar"
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-100">
                        ...
                        <span className="text-xs font-bold text-slate-800">
                          {new Date(
                            activeTicket.sender_created_at || Date.now(),
                      ...
                      <div className="flex flex-col gap-3">
                        {activeDirectChat.isBlockedByMe ? (
                          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                            ...
                            <button
                              onClick={() =>
                                handleUnblockDirectChat(activeDirectChat.chatId)
                              }
                          ...
                          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <button
                                onClick={() =>
                                  handleBlockDirectChat(
                                    activeDirectChat.chatId,
                                    1,
                                  )
                                }
                                className="py-2 bg-white border border-slate-200 hover:border-amber-400 hover:bg-amber-50 text-slate-700 rounded-lg text-xs font-bold transition-all"
                        ...
                        {activeDirectChat.isDeleted ? (
                          <button
                            onClick={() =>
                              handleRestoreDirectChat(activeDirectChat.chatId)
                            }
                            className="w-full py-3 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 border border-emerald-200 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all"
                          >
                            <RefreshCw className="w-4 h-4" />
                          ...
                          <button
                            onClick={() =>
                              handleDeleteDirectChat(activeDirectChat.chatId)
                            }
                            className="w-full py-3 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all"
                          >
                            <Trash2 className="w-4 h-4" />
