  ...
  const handleFileDispute = async (e) => {
    ...
    try {
      ...
      await api.post(`/contracts/${contractId}/dispute?freelancerId=${user.id}`, {
        ...
        amount: disputeAmount ? Number(disputeAmount) : contract.agreedAmount
      });
          ...
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl w-full max-w-md border border-slate-200 shadow-xl p-6 animate-in zoom-in-95 duration-150">
              <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-100">
                ...
                <button onClick={() => { setShowDisputeModal(false); setDisputeReason(''); setDisputeAmount(''); }} className="text-slate-400 hover:text-slate-600">
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <form onSubmit={handleFileDispute} className="space-y-4">
                <label className="block">
                  <span className="block text-[11px] font-bold text-slate-600 uppercase mb-1">Mức độ ưu tiên *</span>
                  <select
                    value={disputePriority}
                    onChange={(e) => setDisputePriority(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs font-semibold text-slate-800 outline-none transition focus:border-rose-500 focus:bg-white mb-4"
                  >
                    <option value="HIGH">Gấp / Nghiêm trọng</option>
                    <option value="MEDIUM">Trung bình</option>
                    <option value="LOW">Bình thường</option>
                  </select>
                </label>
                <label className="block">
                  <span className="block text-[11px] font-bold text-slate-600 uppercase mb-1">Số tiền khiếu nại (VNĐ)</span>
                  <input
                    type="number"
                    placeholder={`Tối đa: ${contract.agreedAmount}`}
                ...
                <div className="flex justify-end gap-2 pt-2 border-t border-slate-50">
                  <button
                    type="button"
                    onClick={() => { setShowDisputeModal(false); setDisputeReason(''); setDisputeAmount(''); }}
