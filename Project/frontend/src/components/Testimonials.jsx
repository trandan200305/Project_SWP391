...
export default function Testimonials() {
  // Mock Top Freelancers from user's vLance screenshot
  const freelancers = [
    {
      name: 'Minh Anh',
      role: 'UI/UX Designer • 5+ năm',
      rating: '4.9',
      reviews: '126',
      avatarColor: 'bg-teal-600 text-teal-100',
      initials: 'MA',
      verified: true
    },
    {
      name: 'Quang Huy',
      role: 'Backend Developer • PHP/Java',
      rating: '5.0',
      reviews: '86',
            ...
            {freelancers.map((freelancer, index) => (
              ...
              >
                ...
                <div className="relative mb-4">
                  <div className={`w-20 h-20 ${freelancer.avatarColor} rounded-full flex items-center justify-center font-extrabold text-2xl shadow-md`}>
                    {freelancer.initials}
                  </div>
                  {freelancer.verified && (
                ...
                {/* Role */}
                <span className="text-body-sm text-muted mb-3 block font-medium">{freelancer.role}</span>
                
                {/* Ratings */}
                <div className="flex items-center justify-center gap-1.5 mb-6">
                  <Star className="w-4 h-4 text-amber-500 fill-amber-500" />
                  <span className="font-bold text-primary text-body-sm">{freelancer.rating}</span>
                  <span className="text-muted text-[13px]">({freelancer.reviews} đánh giá)</span>
                </div>
