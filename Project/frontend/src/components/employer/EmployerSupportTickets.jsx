...
export default function EmployerSupportTickets({ user, defaultOpenModal = false }) {
  ...
  const [category, setCategory] = useState('PAYMENT'); // PAYMENT, SYSTEM, GENERAL, OTHER
  ...
  const fetchTickets = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`http://localhost:8080/api/chat/tickets/employer/${employerId}`);
      if (!res.ok) {
  ...
  const handleCreateTicket = async (e) => {
    ...
    const categoryPrefixMap = {
      PAYMENT: '[Lỗi nạp tiền / Thanh toán]',
      SYSTEM: '[Lỗi kỹ thuật / Hệ thống]',
      GENERAL: '[Thắc mắc chung & Tư vấn]',
      OTHER: '[Khác]'
    };
    ...
    try {
      const res = await fetch('http://localhost:8080/api/chat/tickets/employer', {
        method: 'POST',
  ...
  const handleSendReply = async (e) => {
    ...
    try {
      const payload = {
        ...
        senderId: employerId,
        senderRole: 'EMPLOYER',
        senderName: user?.displayName || user?.companyName || 'Employer',
        senderAvatar: user?.avatarUrl || user?.companyLogoUrl || '',
  ...
  // Close / Resolve Ticket by Employer
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 rounded-2xl p-6 text-white shadow-level-2 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/20 text-cyan-300 text-xs font-bold mb-3 border border-cyan-400/30">
              <LifeBuoy className="w-3.5 h-3.5" />
              Trung tâm Hỗ trợ kỹ thuật & Sự cố
            </div>
            <h2 className="text-2xl font-extrabold tracking-tight">Hỗ trợ & Xử lý Sự cố Employer</h2>
            <p className="text-sm text-slate-300 mt-1 max-w-xl leading-relaxed">
              Gửi ticket yêu cầu khi bạn gặp sự cố nạp tiền, lỗi hệ thống hoặc cần giải đáp thắc mắc. Đội ngũ Staff sẽ tiếp nhận và phản hồi trực tiếp dữ liệu tới CSDL.
            </p>
          </div>
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2.5 px-5 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-extrabold text-sm hover:from-cyan-400 hover:to-blue-500 shadow-md shadow-cyan-950/40 transition-all hover:scale-[1.02] active:scale-[0.98] shrink-0"
          >
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-1 sm:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 rounded-xl border border-slate-200 text-xs font-medium focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500"
            />
          </div>
          <button
            type="button"
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center">
          ...
          <p className="text-xs text-slate-500 mt-1 max-w-md mx-auto">
            {searchQuery
              ...
              : 'Bạn chưa tạo ticket hỗ trợ nào. Khi cần giải quyết sự cố nạp tiền hoặc câu hỏi hệ thống, hãy tạo ticket hỗ trợ mới.'}
          </p>
          {!searchQuery && (
            <button
              type="button"
          ...
          {filteredTickets.map((ticket) => (
            ...
            >
              <div className="space-y-2 flex-1">
                ...
                <h4 className="text-base font-extrabold text-slate-900 hover:text-cyan-700 transition-colors cursor-pointer"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white border border-slate-200 rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-6 relative max-h-[90vh] overflow-y-auto">
            ...
            <div>
              ...
              <h3 className="text-lg font-extrabold text-slate-900">Gửi Ticket Hỗ trợ kỹ thuật & Sự cố</h3>
            ...
            <form onSubmit={handleCreateTicket} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`flex items-center gap-2 p-2.5 rounded-xl border text-left text-xs font-bold transition-all ${
                      category === 'PAYMENT'
                        ? 'border-cyan-500 bg-cyan-50/50 text-cyan-800 ring-1 ring-cyan-500'
                        : 'border-slate-200 hover:bg-slate-50 text-slate-700'
                    }`}
                  >
                    <DollarSign className="w-4 h-4 text-cyan-600 shrink-0" />
                    <span>Lỗi nạp / Thanh toán</span>
                  </button>
                  ...
                  >
                    ...
                    <span>Lỗi hệ thống / Kỹ thuật</span>
                  </button>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-slate-200 rounded-xl text-xs font-semibold focus:border-cyan-500 focus:outline-none"
                >
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="Ví dụ: Đã chuyển khoản nạp tiền nhưng số dư ví chưa cộng..."
                  value={subject}
                  onChange={(e) => setSubject(e.target.value)}
                  className="w-full px-3 py-2 border border-slate-200 rounded-xl text-xs font-medium focus:border-cyan-500 focus:outline-none"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  required
                  rows={4}
                  placeholder="Mô tả cụ thể mã giao dịch, thời gian gặp lỗi, ảnh minh họa hoặc câu hỏi..."
                  value={description}
              ...
              {/* Attachments */}
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1.5">Tệp đính kèm (Ảnh/Hóa đơn bằng chứng)</label>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl max-w-3xl w-full h-[85vh] shadow-2xl flex flex-col overflow-hidden relative">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50/50">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs mb-6 space-y-2">
                <span className="text-[11px] font-extrabold text-cyan-700 bg-cyan-50 px-2 py-0.5 rounded border border-cyan-200">
                ...
                chatMessages.map((msg) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex gap-3 max-w-[80%] ${isEmployer ? 'ml-auto flex-row-reverse' : 'mr-auto'}`}
                    >
                      <div className="w-8 h-8 rounded-full bg-slate-200 overflow-hidden shrink-0 flex items-center justify-center text-slate-600 text-xs font-bold shadow-xs">
                        {msg.senderAvatar ? (
                          ...
                          msg.senderName?.charAt(0) || (isEmployer ? 'E' : 'S')
                        )}
                      </div>

                      <div className="space-y-1">
                        <div className={`flex items-center gap-2 text-[11px] font-bold ${isEmployer ? 'justify-end text-cyan-900' : 'text-slate-700'}`}>
                          <span>{msg.senderName || (isEmployer ? 'Bạn (Employer)' : 'Staff Hỗ trợ')}</span>
                            ...
                            <div className="mt-2.5 pt-2 border-t border-slate-700/30 space-y-1">
                              {msg.attachments.map((att, aIdx) => (
                                <a
                                  ...
                                  className={`flex items-center gap-2 text-[11px] hover:underline ${isEmployer ? 'text-cyan-200' : 'text-cyan-700'}`}
                                >
              ...
              <form onSubmit={handleSendReply} className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 text-xs font-medium focus:border-cyan-500 focus:outline-none"
                  />
