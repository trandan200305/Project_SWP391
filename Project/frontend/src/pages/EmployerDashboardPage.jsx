...
export default function EmployerDashboardPage({ user, onNavigate }) {
  ...
  // Hired freelancers modal states
  const [showHiredFreelancersModal, setShowHiredFreelancersModal] = useState(false);
  const [hiredFreelancerSearch, setHiredFreelancerSearch] = useState('');
  ...
  return (
    <div className="min-h-screen bg-slate-50/80 pb-16">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-6 space-y-5">
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                ...
                <button
                  onClick={() => {
                    if (!isRecommendation && favoriteFreelancers.length > 0) {
                      setShowHiredFreelancersModal(true);
                    } else {
                ...
                >
                  Khám phá thêm <ChevronRight className="w-3 h-3" />
                </button>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h2 className="text-sm font-bold text-slate-900">Gói cước dịch vụ</h2>
                    <p className="text-[10px] text-slate-500">Thông tin gói & quyền lợi bài đăng</p>
                  </div>
                </div>
                <button
                  onClick={() => onNavigate('employer_packages')}
              ...
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-200/70 space-y-2.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-extrabold text-indigo-600">{packageInfo?.currentPackageName || (tier !== 'ĐỒNG' ? `Hạng ${tier}` : 'Tiêu Chuẩn')}</span>
                </div>
                <div className="flex items-center justify-between border-t border-slate-200/60 pt-2">
                  <span className="text-xs text-slate-500 font-medium">Đăng bài còn lại:</span>
                  <span className="text-xs font-extrabold text-emerald-600">{packageInfo?.remainingPostsDisplay || 'Không giới hạn'}</span>
                </div>
                <div className="flex items-center justify-between border-t border-slate-200/60 pt-2 text-[11px]">
                  ...
                  <span className="font-semibold text-slate-700">{packageInfo?.currentPackageExpiry || 'Không giới hạn'}</span>
                </div>
              </div>

              <button
                onClick={() => onNavigate('employer_packages')}
                className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl shadow transition flex items-center justify-center gap-1.5 cursor-pointer"
              >
                <Sparkles className="w-3.5 h-3.5" />
                <span>Quản lý & Lịch sử gói cước</span>
              </button>
            </div>
      ...
      {/* Hired Freelancers Modal Popup */}
      {showHiredFreelancersModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[85vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-900">Freelancer đã từng hợp tác</h3>
                  <p className="text-xs text-slate-500">Danh sách tất cả ứng viên bạn đã từng hợp tác và nghiệm thu dự án thành công</p>
                </div>
              </div>
              <button
                onClick={() => setShowHiredFreelancersModal(false)}
            ...
            <div className="p-4 border-b border-slate-100 bg-white">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Tìm kiếm tên Freelancer hoặc chuyên môn..."
                  value={hiredFreelancerSearch}
                  onChange={(e) => setHiredFreelancerSearch(e.target.value)}
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                />
              </div>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-3 flex-1">
              {favoriteFreelancers
                .filter(f => {
                  const key = (hiredFreelancerSearch || '').trim().toLowerCase();
                  if (!key) return true;
                  return (
                    (f.display_name && f.display_name.toLowerCase().includes(key)) ||
                    (f.professional_title && f.professional_title.toLowerCase().includes(key))
                  );
                })
                .map((free) => (
                  <div key={free.freelancer_id} className="flex items-center justify-between gap-4 p-4 bg-slate-50/70 hover:bg-indigo-50/30 rounded-2xl border border-slate-200/80 transition">
                    <div className="flex items-center gap-3.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <h4
                            onClick={() => {
                              setShowHiredFreelancersModal(false);
                              onNavigate('freelancer_profile', { freelancerId: free.freelancer_id });
                            }}
                            className="font-bold text-sm text-slate-900 hover:text-indigo-600 cursor-pointer truncate"
                          >
                            {free.display_name}
                          </h4>
                          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-indigo-50 text-indigo-700 border border-indigo-100 shrink-0">
                            {free.total_contracts} hợp đồng
                          </span>
                        </div>
                        <p className="text-xs text-slate-500 truncate mt-0.5">{free.professional_title || 'Freelancer Professional'}</p>
                        <div className="flex items-center gap-3 text-xs text-slate-400 mt-1">
                          <span className="text-amber-600 font-bold flex items-center gap-1">★ {free.average_rating ? Number(free.average_rating).toFixed(1) : '5.0'}</span>
                          {free.city && <span>• {free.city}</span>}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <button
                        onClick={() => {
                          setShowHiredFreelancersModal(false);
                          onNavigate('messenger', { id: free.freelancer_id, role: 'FREELANCER', name: free.display_name, avatarUrl: free.avatar_url });
                        }}
                        className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-xl transition flex items-center gap-1.5 shadow-sm"
                      >
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100 text-right">
              <button
                onClick={() => setShowHiredFreelancersModal(false)}
                className="px-5 py-2 bg-white hover:bg-slate-100 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 transition"
              >
                Đóng
              </button>
            </div>
