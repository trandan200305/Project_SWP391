const fs = require('fs');
const file = 'c:/Users/admin/Downloads/Project_SWP391/Project_SWP391/Project/frontend/src/features/admin/pages/AdminDashboardPage.jsx';
let content = fs.readFileSync(file, 'utf8');
const lines = content.split('\n');

const newAside =       <aside className="w-sidebar-width bg-surface border-r border-outline-variant flex flex-col justify-between shrink-0 hidden md:flex h-full">
...
const newHeader =         <header className="flex justify-between items-center h-16 px-margin-page sticky top-0 w-full bg-surface/80 backdrop-blur-md z-40 border-b border-outline-variant">
...
for (let i = 0; i < lines.length; i++) {
    if (i >= 2597 && i <= 2742) {
        if (i === 2597) {
            outLines.push(newAside);
        }
    } else if (i >= 2748 && i <= 2915) {
        if (i === 2748) {
            outLines.push(newHeader);
        }
    } else {
