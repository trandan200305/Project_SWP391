# SOLELY FOR ADMIN ROLE - SYSTEM TEST CASES

*Tôi đã gom TẤT CẢ các kịch bản kiểm thử dành riêng cho chức năng của người Quản trị (Admin) vào một bảng duy nhất ở dưới đây. Nếu bạn làm việc theo nhóm và được phân công làm trang `ROLE ADMIN`, bạn CHỈ CẦN copy duy nhất bảng này dán vào sheet của bạn là đủ!*

## 📋 Mời copy bảng này dán vào Sheet ROLE ADMIN của bạn:

| Test Case ID | Test Case Description | Test Case Procedure | Expected Results | Pre-conditions | Round 1 | Test date | Tester | Round 2 | Test date | Tester | Round 3 | Test date | Tester | Note |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **Scenario: Admin Authentication & Security** | | | | | | | | | | | | | | |
| TC_ADM_001 | Admin Login Valid | 1. Access /admin/login<br>2. Enter valid Admin Email & Password<br>3. Submit | Login successful, redirects to Admin Dashboard | Admin account exists | Pending | | | Pending | | | Pending | | | |
| TC_ADM_002 | Admin Login Invalid Pass | 1. Enter valid Admin Email & invalid Password<br>2. Submit | Displays error: "Incorrect account or password" | Admin account exists | Pending | | | Pending | | | Pending | | | |
| TC_ADM_003 | Admin Login Empty Fields | 1. Leave Email & Password blank<br>2. Submit | Submit button disabled or UI shows "This field is required" | None | Pending | | | Pending | | | Pending | | | |
| TC_ADM_004 | Admin Panel SQL Injection | 1. Enter SQL injection payload in Admin Email (e.g., ' OR 1=1--)<br>2. Submit | Login rejected, database remains secure | None | Pending | | | Pending | | | Pending | | | |
| TC_ADM_005 | Admin Panel XSS Payload | 1. Enter XSS payload in Email (<script>alert(1)</script>)<br>2. Submit | Format rejected or backend sanitizes input, login fails | None | Pending | | | Pending | | | Pending | | | |
| TC_ADM_007 | Admin Panel Invalid Role Access | 1. Attempt to login to Admin Panel using a Freelancer/Employer account | Displays error "Access denied for administrator role" (403 Forbidden) | Freelancer account exists | Pending | | | Pending | | | Pending | | | |
| TC_ADM_008 | Admin Session Expiry | 1. Login as Admin<br>2. Wait for JWT token to expire<br>3. Navigate to another tab | 401 Unauthorized, automatically redirects Admin to Login page | Admin is logged in | Pending | | | Pending | | | Pending | | | |
| TC_ADM_009 | Admin Concurrent Login | 1. Login with the same Admin account on 2 different browsers simultaneously | System handles session gracefully (either both allowed or oldest session is kicked out) | Admin account exists | Pending | | | Pending | | | Pending | | | |
| **Scenario: Admin - Staff & Manager Oversight** | | | | | | | | | | | | | | |
| TC_ADM_010 | Admin Invites Valid Staff | 1. Navigate to Staff Management<br>2. Click Add New<br>3. Enter valid email and select Staff role<br>4. Submit | Staff account created, invitation email sent | Logged in as Admin | Pending | | | Pending | | | Pending | | | |
| TC_ADM_011 | Admin Invites Duplicate Email | 1. Try to invite an email that already exists in the system | Displays error "Email already exists in the system" | Logged in as Admin | Pending | | | Pending | | | Pending | | | |
| TC_ADM_012 | Admin Invites Invalid Email Format | 1. Enter incorrectly formatted email (e.g., admin#gmail,com) | Validation error: "Invalid email format" | Logged in as Admin | Pending | | | Pending | | | Pending | | | |
| TC_ADM_013 | Admin Resets User Pass Normal | 1. Select a Staff user<br>2. Force reset password to "Admin123!" | Password successfully updated, user can login with new password | Logged in as Admin | Pending | | | Pending | | | Pending | | | |
| TC_ADM_014 | Admin Resets User Pass Weak | 1. Force reset password to a 1-character string | Validation error: "Password must be at least 6 characters" | Logged in as Admin | Pending | | | Pending | | | Pending | | | |
| TC_ADM_016 | Admin Views Manager List | 1. Navigate to Manager Workspaces tab | Displays list of all Manager accounts with their status and activity logs | Logged in as Admin | Pending | | | Pending | | | Pending | | | |
| TC_ADM_017 | Admin Revokes Manager Access | 1. Select an active Manager<br>2. Click Revoke Access<br>3. Confirm | Manager status changes to BANNED/REVOKED, their current session is terminated | Logged in as Admin, Manager exists | Pending | | | Pending | | | Pending | | | |
| **Scenario: Admin - Dashboard Analytics** | | | | | | | | | | | | | | |
| TC_ADM_018 | Load Stats Normal | 1. Open Admin Dashboard<br>2. Observe Total Revenue & Orders (30 days filter) | Data loads accurately matching the database records | Logged in as Admin | Pending | | | Pending | | | Pending | | | |
| TC_ADM_019 | Zero Data Handling | 1. Connect backend to an empty database (0 transactions)<br>2. Load Admin Dashboard | No "Divide by Zero" errors, displays 0 Revenue, 0 Orders, chart shows "No data" | Empty database | Pending | | | Pending | | | Pending | | | |
| TC_ADM_020 | Spam Filter Clicks | 1. Rapidly click between "Today", "Week", "Month" filters on Admin Dashboard | UI does not freeze/crash, API requests are debounced, final render matches the last clicked filter | Logged in as Admin | Pending | | | Pending | | | Pending | | | |
...
| **Scenario: Admin - Financial Configuration** | | | | | | | | | | | | | | |
...
| **Scenario: Admin - Service Packages** | | | | | | | | | | | | | | |
...
| **Scenario: Admin - VNPay Transactions** | | | | | | | | | | | | | | |
...
| **Scenario: Admin - Moderation (KYC & Projects)** | | | | | | | | | | | | | | |
...
| **Scenario: Admin - Dispute Resolution** | | | | | | | | | | | | | | |
...
| **Scenario: Admin - Withdrawals Processing** | | | | | | | | | | | | | | |
